        ...
        for local_wheel_workspace in local_wheel_workspaces:
            ...
            if dist_folder_path.exists:
                ...
                _process_dist_wheels(
                    ...
                    py_ver_marker,
                    py_major_ver_marker,
                    ctx.attr.local_wheel_whitelist,
                    ctx.attr.local_wheel_blacklist,
                )
    if local_wheels_dir:
...
python_repository = repository_rule(
    ...
    attrs = {
        ...
        "local_wheel_whitelist": attr.string_list(
            mandatory = False,
            default = ["*"],
        ),
        "local_wheel_blacklist": attr.string_list(
            mandatory = False,
            default = [],
        ),
    },
...
def _process_dist_wheels(
        ...
        py_ver_marker,
        py_major_ver_marker,
        local_wheel_whitelist,
        local_wheel_blacklist):
    for wheel in dist_wheels:
        bn = wheel.basename
        if not bn.endswith(".whl") or (bn.find(py_ver_marker) < 0 and bn.find(py_major_ver_marker) < 0):
            continue
        if not _basic_wildcard_match(bn, local_wheel_whitelist, True, False):
            continue
        if not _basic_wildcard_match(bn, local_wheel_blacklist, False, True):
            continue
