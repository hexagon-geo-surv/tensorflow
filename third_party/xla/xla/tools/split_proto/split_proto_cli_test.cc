...
TEST(SplitProtoCliTest, PackExecutableSpecialized) {
  auto initial_proto =
  ...
  PackOptions pack_opts;
  pack_opts.input_format = ProtoFormat::kText;
  ASSERT_OK(PackExecutable(riegeli::Maker<riegeli::StringReader>(text_input),
                           riegeli::Maker<riegeli::StringWriter>(&split_bytes),
                           pack_opts));
...
TEST(SplitProtoCliTest, UnpackExecutableSpecialized) {
