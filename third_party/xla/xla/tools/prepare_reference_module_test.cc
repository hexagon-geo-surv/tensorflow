...
// Ideally 'Deoptimizer' pass should be mocked. Because it is not feasible with
// the current design, deoptimization tests in this file base on deoptimizer's
// implementation (deoptimizer removes fusion op from the module).
TEST_F(PrepareReferenceModuleTest, PerformDeoptimization) {
...
TEST_F(PrepareReferenceModuleTest, SkipDeoptimization) {
  TF_ASSERT_OK_AND_ASSIGN(auto test_module,
                          ParseAndReturnVerifiedModule(kModuleStr));

  TF_ASSERT_OK_AND_ASSIGN(auto reference_module,
                          PrepareReferenceModule(*test_module, nullptr, {}, {},
                                                 /*skip_deoptimization=*/true));
