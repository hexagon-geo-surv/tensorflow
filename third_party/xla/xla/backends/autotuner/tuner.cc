...
absl::StatusOr<std::unique_ptr<Tuner>> Tuner::Create(
    std::unique_ptr<Profiler> profiler, CodegenOrchestrator* orchestrator,
    Options options) {
...
tsl::Future<Tuner::Config> Tuner::GetTunedConfig(HloInstruction* instr) {
  auto supported_configs_or = orchestrator_->GetSupportedConfigs(instr);
  if (!supported_configs_or.ok()) {
    return supported_configs_or.status();
  }
  std::vector<Config> supported_configs = std::move(*supported_configs_or);
  ...
  auto executables = orchestrator_->CompileAll(instr, supported_configs);
