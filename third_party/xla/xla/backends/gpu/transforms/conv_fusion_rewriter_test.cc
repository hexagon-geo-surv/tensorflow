...
TEST_F(ConvFusionRewriterUnitTest, ConvertF32ToF64PrologueNotFused) {
  // cuDNN graph API does not support double-precision convolutions, so f64
  // convolutions are not fused.
  RunAndMatch(
      R"(
    HloModule Test
...
TEST_F(ConvFusionRewriterUnitTest, F32HighestPrecisionNotFused) {
  // cuDNN graph API does not support disabling TF32 for f32 convolutions,
  // so highest precision f32 convolutions are not fused.
  RunAndMatch(
      R"(
    HloModule Test

    ENTRY Test {
      input = f32[4,48,96,64] parameter(0)
      filter = f32[128,3,3,64] parameter(1)
      ROOT conv = f32[4,24,48,128] convolution(input, filter),
                    window={size=3x3 stride=2x2 pad=0_1x0_1},
                    dim_labels=b01f_o01i->b01f,
