...
class DevicelessEstimateCubScratchSizeTest
    : public HloHardwareIndependentTestBase {
 protected:
  absl::StatusOr<int64_t> RunPassAndExtractScratchSize(
      absl::string_view hlo_text, EstimateCubScratchSize& pass) {
...
TEST_F(DevicelessEstimateCubScratchSizeTest, ReturnsLookupTableSize_KeyOnly) {
  ...
  EstimateCubScratchSize::DevicelessOptions opts{"NVIDIA H100 80GB HBM3",
                                                 SemanticVersion(3, 1, 2)};
  EstimateCubScratchSize pass("CUDA", opts);
...
TEST_F(DevicelessEstimateCubScratchSizeTest, ReturnsLookupTableSize_KeyValue) {
  ASSERT_OK_AND_ASSIGN(const CubScratchSizeDevicelessLookup& lookup_table,
                       CubScratchSizeDevicelessLookup::GetInstance());
  absl::string_view hlo = R"hlo(
    HloModule m
...
TEST_F(DevicelessEstimateCubScratchSizeTest, ReturnsLookupTableSize_Batched) {
  ASSERT_OK_AND_ASSIGN(const CubScratchSizeDevicelessLookup& lookup_table,
                       CubScratchSizeDevicelessLookup::GetInstance());
  absl::string_view hlo = R"hlo(
    HloModule m
