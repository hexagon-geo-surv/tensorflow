...
template <typename T, std::int64_t kAlignment>
...
absl::string_view AsPtxString(const void* kernel_data, int64_t kernel_size) {
  // NOLINTNEXTLINE
  return absl::string_view(reinterpret_cast<const char*>(kernel_data),
                           kernel_size);
}

absl::Span<const uint8_t> AsCubin(const void* kernel_data,
                                  int64_t kernel_size) {
...
class FfiKernelCache : public se::CommandBuffer::Resource {
 private:
  ...
  struct KernelKey {
    ...
    struct Hash {
      ...
      size_t operator()(
          const std::pair<absl::string_view, const void*>& k) const {
 ...
 public:
  absl::StatusOr<se::Kernel*> GetOrCreateKernel(
    ...
    if (it != kernels_.end()) {
      VLOG(3) << "FfiKernelCache: found kernel: " << kernel_name_view
              << ", ptr: " << it->second.get();
    ...
    se::KernelLoaderSpec spec =
        is_ptx
            ? se::KernelLoaderSpec::CreateCudaPtxInMemorySpec(
                  AsPtxString(kernel_data, kernel_size), kernel_name, num_args)
            : se::KernelLoaderSpec::CreateCudaCubinInMemorySpec(
                  AsCubin(kernel_data, kernel_size), kernel_name, num_args);
    ...
    VLOG(3) << "FfiKernelCache: created kernel: " << kernel_name
            << ", ptr: " << kernel_ptr;
    return kernel_ptr;
  }
