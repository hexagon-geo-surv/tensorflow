...
absl::Status RecordFfiHandler(ffi::RecordContext record_ctx,
  ...
  if (action == ffi::RecordAction::kCreate) {
    RETURN_IF_ERROR(record_ctx.CreateMemcpyD2D(scratch, in0, size).status());
    ...
    RETURN_IF_ERROR(record_ctx.UpdateMemcpyD2D(cmds[0], scratch, in0, size));
    RETURN_IF_ERROR(record_ctx.UpdateLaunch(
