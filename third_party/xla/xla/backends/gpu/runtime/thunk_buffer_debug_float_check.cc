...
// unchanged.
//
// `printed_instructions` is a cache used to avoid printing the same
...
FloatCheckReportResult ReportFloatCheckResult(
  ...
  if (!(enabled_checks.check_nans && has_nans &&
        !reported_nans.contains(metadata.profile_annotation)) &&
      !(enabled_checks.check_infs && has_infs &&
        !reported_infs.contains(metadata.profile_annotation)) &&
      !enabled_checks.check_minmax) {
...
EnabledChecks GetEnabledChecks(const HloModule* absl_nonnull hlo_module) {
  ...
  return EnabledChecks{
      ...
      /*check_infs_fatal=*/inf_detection_mode ==
          DebugOptions::DETECTION_MODE_FAIL,
      /*check_minmax=*/
      hlo_module->config().debug_options().xla_gpu_print_minmax(),
  };
}
