...
CreateOutputBuffersCheckThunk(
  ...
  absl::flat_hash_map<size_t, BufferAllocation::Slice> buffers_to_check;
  buffers_to_check.reserve(output_slices.size());
  size_t max_buffer_size_bytes = 0;

  for (const auto& [idx, shaped_slice] : output_slices) {
...
absl::Status RunFloatCheckPassInternal(
  ...
  ASSIGN_OR_RETURN(
      ...
      CreateOutputBuffersCheckThunk(debug_options, hlo_module, output_slices,
                                    log_slice, metadata_store, allocator));
