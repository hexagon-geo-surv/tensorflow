...
absl::Status AddI32FfiHandler(ffi::RecordExtension::Context record_ctx,  //
...
struct FfiRecordTestSetup {
  ...
  static absl::StatusOr<std::unique_ptr<FfiRecordTestSetup>> Create(
    ...
    ASSIGN_OR_RETURN(setup->stream, executor->CreateStream());
    setup->alloc = std::make_unique<RecordTestAlloc>(executor);

    ASSIGN_OR_RETURN(auto slices, AllocateAndCopy(*setup->stream, *setup->alloc,
                                                  inputs, outputs_init));
    ...
    RETURN_IF_ERROR(setup->thunk->Initialize(init_params));
...
absl::Status AddI32WithBarrierFfiHandler(
    ffi::RecordExtension::Context record_ctx, ffi::AnyBuffer input_0,
    ffi::AnyBuffer input_1, ffi::Result<ffi::AnyBuffer> result) {
  if (record_ctx.action() == ffi::RecordAction::kCreate) {
    ...
    const std::vector<void*> args = {input_0.untyped_data(),
                                     input_1.untyped_data(),
                                     (*result).untyped_data()};
    RETURN_IF_ERROR(record_ctx.UpdateLaunch(record_ctx.commands()[0], args));
    RETURN_IF_ERROR(record_ctx.UpdateLaunch(record_ctx.commands()[1], args));
  }
  return absl::OkStatus();
}
