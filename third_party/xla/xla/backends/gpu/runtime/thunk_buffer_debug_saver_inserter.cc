...
absl::Status RunDebugSaverInserter(ThunkSequence* thunk_sequence,
  ...
  if (debug_options.xla_gpu_experimental_thunk_buffer_debug_module_outputs() &&
    ...
    absl::c_sort(sorted_output_buffers, [](const auto& a, const auto& b) {
      return a.first < b.first;
    });

    std::string metadata_str =
        absl::StrCat("Module ", hlo_module.name(), " Output Check");
    std::string profile_annotation_str = absl::StrCat(
        "Buffer saver Module ", hlo_module.name(), " Output Check");
    ...
    for (const auto& [idx, shaped_slice] : sorted_output_buffers) {
      ...
      ASSIGN_OR_RETURN(auto log_thunk,
                       CustomCallThunk::Create(
                           info, std::string{kXlaGpuAppendToFileCustomCallTag},
                           {shaped_slice}, {std::nullopt}, attributes,
                           hlo_module.entry_computation(), "GPU",
                           stream_executor::GpuComputeCapability()));
