...
BlockLevelEmitterBackend::GetSupportedConfigs(const HloInstruction& instr) {
  ...
  absl::StatusOr<TopKTiledRunTimeDataOrError> tiled_runtime_data_or_error =
      indexing_performance_model_.TryFindBestTilingForFusion(
          *fusion_adaptor, instr.GetModule()
                               ->config()
                               .debug_options()
                               .xla_gpu_fusion_autotune_top_k_configs());

  if (!tiled_runtime_data_or_error.ok() ||
      std::holds_alternative<FusionDecision>(
          tiled_runtime_data_or_error.value())) {
    return std::vector<std::unique_ptr<BackendConfig>>();
  }
