...
This tool autotunes an HLO module from a given HLO file and cache specified by
cache_path. It respects autotuning related XLA_FLAGS except the cache related flags.
  ...
  for (int i = 0; i < device_count; ++i) {
    ...
    if (profiler == nullptr) {
      return absl::InternalError(
          absl::StrCat("Failed to create profiler for device ", i));
    }
