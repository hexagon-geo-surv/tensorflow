...
TEST_F(TritonBackendTest, WarpSpecializationWithCastDoesNotCrash) {
  if (target_config_.device_description.gpu_compute_capability().IsRocm()) {
    GTEST_SKIP() << "Not supported on ROCm.";
  }
...
%gemm_fusion_dot.8888_computation {
  %p1 = bf16[64,32,128]{2,1,0} parameter(1)
  %p0 = bf16[32,64,32]{2,1,0} parameter(0)
...
ENTRY %entry_computation {
  %p0 = bf16[32,64,32]{2,1,0} parameter(0)
  %p1 = bf16[64,32,128]{2,1,0} parameter(1)
  ROOT %micro_kernel = f32[64,32,128]{2,1,0} fusion(%p0, %p1), kind=kCustom, calls=%gemm_fusion_dot.8888_computation, backend_config={"operation_queue_id":"0","fusion_backend_config":{"kind":"__triton_gemm"},"force_earliest_schedule":false,"reification_cost":[],"device_type":"DEVICE_TYPE_INVALID"}
})";
