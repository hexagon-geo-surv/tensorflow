...
TEST_F(CudnnBackendTest, GetDefaultConfigSucceedsWithNullStreamExecutor) {
  ...
  ASSERT_OK_AND_ASSIGN(Compiler::GpuTargetConfig deviceless_config,
  ...
  CudnnBackend backend_without_stream_executor(/*stream_executor=*/nullptr,
                                               &debug_options, &compiler_,
                                               &deviceless_config);
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> hlo_module,
                       ParseAndReturnVerifiedModule(kCudnnFusionHlo));
