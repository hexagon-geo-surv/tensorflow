...
class RewriteExtract : public mlir::OpRewritePattern<ExtractOp> {
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
      ...
      auto descriptor_load =
          builder
              .create<DescriptorLoadOp>(
                  ...
                  IndexCastUI(builder, builder.getI32Type(),
                              op.getOffsetsAsValues(builder)))
              .getResult();
    ...
    auto ptr = CreateAddPtrOp(builder, op.getSrc(),
                              op.getOffsetsAsValues(builder), op.getLayout());
    ptr = CreateMakeTensorPtrOp(builder, ptr, original_shape, tile_shape,
                                op.getOffsetsAsValues(builder),
                                op.getStridesAsValues(builder), op.getLayout());
 ...
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (CanUseTMA(builder, tma_enabled_, *device_description_, tile_shape,
      ...
      ptr = CreateMakeTensorPtrOp(builder, ptr, original_shape, tile_shape,
                                  offsets, op.getStridesAsValues(builder),
                                  op.getLayout());
