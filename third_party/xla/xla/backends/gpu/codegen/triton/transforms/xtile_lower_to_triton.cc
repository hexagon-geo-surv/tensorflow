 ...
 private:
  static SmallVector<Value> CloneBlock(
    ...
    for (mlir::Operation& op_in_block : old_block.without_terminator()) {
      if (isa<tensor::ExtractOp, tensor::FromElementsOp>(&op_in_block)) {
      ...
      if (op_in_block.hasTrait<mlir::OpTrait::ConstantLike>()) {
        ...
        if (broadcast_shape) {
          ...
          mapping.map(op_in_block.getResult(0), cloned_const->getResult(0));
        }
        continue;
      }
...
class XTileLowerToTritonPass
 ...
 public:
  void runOnOperation() override {
    ...
    patterns.add<CanonicalizeDotScaled, LowerDotScaled,  //
                 LowerReshape, LowerScan>(mlir_context);
    if (mlir::failed(
