...
xla::Future<LlvmKernelSource> MlirKernelFusion::CreateLLVMModule(
    ...
    KernelCompiler* kernel_compiler) const {
  BorrowedMlirContext borrowed_context = BorrowMlirContext();
...
BorrowedMlirContext BorrowMlirContext() {
  static absl::NoDestructor<ObjectPool<std::unique_ptr<mlir::MLIRContext>>>
      pool([]() { return CreateMlirContext(); });
  auto context = pool->GetOrCreate();
