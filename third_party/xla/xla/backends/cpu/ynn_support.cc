  ...
  if (Match(to_apply->root_instruction(),
    ...
    static const absl::NoDestructor<
        ...
        kAllowedTypes({
            ...
            {F16, F32},
            {BF16, F32},
            // YNNPACK accumulates in F32. We should only allow this if we
            // permit excess precision. However, it seems that XLA already uses
            // F32 accumulators in some cases without excess precision, so we
            // are enabling this unconditionally for consistency.
            {BF16, BF16},
            {F16, F16},
            {S8, S32},
            {U8, S32},
...
bool IsInstructionPreferredByYnn(const HloInstruction* instr) {
  ...
  if (HasAtLeastMinElements(instr->shape(), min_elements)) {
    return true;
  }
  for (const HloInstruction* operand : instr->operands()) {
    if (HasAtLeastMinElements(operand->shape(), min_elements)) {
      return true;
    }
  }
  return false;
