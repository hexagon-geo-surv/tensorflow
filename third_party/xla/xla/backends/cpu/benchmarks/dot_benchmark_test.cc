...
void BM_GenericDot(benchmark::State& state, GenericDot info) {
  ...
  auto lhs_shape = ShapeUtil::MakeShape(F32, info.lhs_shape);
  auto rhs_shape = ShapeUtil::MakeShape(F32, info.rhs_shape);
  auto out_shape = ShapeUtil::MakeShape(F32, info.out_shape);
  ...
  double stddev = 0.1f;
  std::minstd_rand0 engine;
  PrimitiveType dtype = F32;
  if (dtype == F32) {
    lhs_lit = *LiteralUtil::CreateRandomLiteral<F32>(lhs_shape, &engine, mean,
                                                     stddev);
    rhs_lit = *LiteralUtil::CreateRandomLiteral<F32>(rhs_shape, &engine, mean,
                                                     stddev);
  } else if (dtype == BF16) {
    lhs_lit = *LiteralUtil::CreateRandomLiteral<BF16>(lhs_shape, &engine, mean,
                                                      stddev);
    rhs_lit = *LiteralUtil::CreateRandomLiteral<BF16>(rhs_shape, &engine, mean,
                                                      stddev);
  } else {
    LOG(FATAL) << "Add dtype to the if-else block before use: " << dtype;
  }
