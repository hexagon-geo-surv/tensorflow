...
absl::StatusOr<ge::TiledHloComputation> GetTiledHloComputationExperimental(
    mlir::MLIRContext& context, const HloFusionInstruction& fusion) {
  RETURN_IF_ERROR(VerifyTensorRanks(fusion));

  auto fusion_adaptor = HloFusionAdaptor::ForInstruction(&fusion);
  auto tiling_space = ge::TilingSpace::Create(*fusion_adaptor, &context);
  auto dims = tiling_space->dimensions();
  ...
  // Cap the dummy tile size to 256 to prevent JIT memory page alignment crashes
  // (max 4KB).
  constexpr int64_t kMaxDummyTileSize = 256;
  for (const auto& dim : dims) {
    int64_t tile_size =
        dim.dimension_size == 0 ? 1 : llvm::PowerOf2Ceil(dim.dimension_size);
    tile_size = std::min(tile_size, kMaxDummyTileSize);
    tile_sizes.push_back(tile_size);
  }

  if (!tiling_space->AssignTileSizes(tile_sizes).ok()) {
    return Internal("Failed to assign tile sizes for: %s", fusion.name());
  }
...
absl::StatusOr<KernelDefinition<MlirKernelSource>> EmitTiledFusionKernelImpl(
  ...
  ASSIGN_OR_RETURN(auto module, xtile::EmitXTileModule(
                                    name, fusion, tiled_computation, context));

  const auto* root = tiled_computation.roots().front();
...
TiledEmissionResult EmitTiledFusionKernel(
  ...
  if (options::EnableExperimentalTiling(fusion.GetModule()->config())) {
    absl::StatusOr<ge::TiledHloComputation> tiled_computation =
        GetTiledHloComputationExperimental(context, fusion);
