...
struct LowerCreateErrorOp : public mlir::OpRewritePattern<CreateErrorOp> {
 public:
  using OpRewritePattern::OpRewritePattern;

  mlir::LogicalResult matchAndRewrite(
      CreateErrorOp op, mlir::PatternRewriter& rewriter) const override {
    auto elementPtrType =
...
void PopulateXlaCpuConversionPatterns(mlir::RewritePatternSet& patterns) {
  patterns.add<LowerLoadOp, LowerStoreOp, LowerThreadId, LowerCreateErrorOp,
