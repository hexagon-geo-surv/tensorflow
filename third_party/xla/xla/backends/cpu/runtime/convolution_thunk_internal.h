...
void EigenConv2D(const EigenDevice& device, ScalarType* out, ScalarType* lhs,
  ...
  if constexpr (use_thread_pool) {
    // Although we schedule at most 16 tasks, note that each individual
    // convolution might also execute in parallel on the same thread pool.
    static constexpr Eigen::Index kMaxTasks = 16;
    auto num_tasks = Eigen::numext::div_ceil(feature_group_count, kMaxTasks);
    auto task_size = Eigen::numext::div_ceil(feature_group_count, num_tasks);
