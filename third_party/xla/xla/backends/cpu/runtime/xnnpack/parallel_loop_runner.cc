...
static ABSL_ATTRIBUTE_ALWAYS_INLINE size_t CoPrime(size_t num_workers) {
...
ComputeOptimalNumWorkers(absl::Duration worker_timeslice, size_t num_threads,
  ...
  return std::min(std::min(num_tasks - 1, num_threads),
...
ABSL_ATTRIBUTE_ALWAYS_INLINE void ParallelLoopRunner::ScheduleAll(
  ...
  // potentially avoid allocating count down counter alltogether.
