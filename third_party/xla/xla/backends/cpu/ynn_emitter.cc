...
absl::StatusOr<uint32_t> DefineIotaOp(ynn_subgraph_t subgraph,
  ...
  YNN_RETURN_IF_ERROR(ynn_define_iota(subgraph, ynn_element_type, 1,
  ...
  if (rank > 1) {
    ...
    CHECK_LE(rank, YNN_MAX_TENSOR_RANK);
    size_t dims[YNN_MAX_TENSOR_RANK];
    std::copy_n(shape.dimensions().data(), rank, dims);
    YNN_RETURN_IF_ERROR(ynn_define_static_broadcast(
        subgraph, rank, dims, full_rank_id, &out_id, /*flags=*/0));
  }
