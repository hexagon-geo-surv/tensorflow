...
class GuardedCallbackAnnotationsAndEvents {
 public:
  CallbackAnnotationsAndEvents Consume() {
    tsl::mutex_lock lock(mu);
    CallbackAnnotationsAndEvents grabbed;
    std::swap(grabbed, annotations_and_events);
    return grabbed;
  }
 ...
 private:
  tsl::mutex mu;
  CallbackAnnotationsAndEvents annotations_and_events TF_GUARDED_BY(mu);
...
absl::Status AddDriverApiCallbackEvent(
  ...
  tsl::mutex_lock lock(guarded_annotations_and_events.mu, std::try_to_lock_t());
  if (!(bool)lock) return absl::OkStatus();
  auto &annotations_and_events =
      guarded_annotations_and_events.annotations_and_events;
  ...
  bool too_many_annotations = tracer->TooManyAnnotationStrings(
      annotations_and_events.CountAnnotations());
