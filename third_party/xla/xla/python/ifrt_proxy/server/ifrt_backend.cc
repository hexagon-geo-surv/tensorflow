 ...
 private:
  ...
  void Loop() {
    ...
    while (auto entry = Pop()) {
      LOG(ERROR) << "Processing " << ++i << entry->req->ShortDebugString();
      parent_->ProcessInternal(std::move(entry->req))
          .OnReady([p = std::move(entry->promise)](
                       absl::StatusOr<Response> r) mutable {
            LOG(ERROR) << "Sending background response1";

            p.Set(std::move(r));
            LOG(ERROR) << "Sending background response1 done";
          });
      LOG(ERROR) << "Finished processing " << i;
    }
  }
...
Future<BackendInterface::Response> IfrtBackend::Process(
  ...
  LOG(ERROR) << "Enqueuing " << request->ShortDebugString();
  auto result = in_order_requests_processor_->Push(std::move(request));
  LOG(ERROR) << "Finished enqueuing";
  return result;
}
