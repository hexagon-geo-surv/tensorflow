...
Array::MakeArrayFromHostBuffer(
  ...
  LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ1";
  ...
  LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ2";
  ...
  if (GetGlobalClientFlags()->synchronous_host_buffer_store ||
    ...
    if (semantics == HostBufferSemantics::kImmutableOnlyDuringCall) {
      ...
      LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ24 mem_region.size()="
                 << mem_region.size()
                 << " mem_region.data()=" << mem_region.data()
                 << " alloc=" << ((void*)alloc);
      LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ25.1";
      memcpy(alloc, mem_region.data(), mem_region.size());
      LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ26";
      mem_region = absl::string_view(alloc, mem_region.size());
      LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ27";
      if (on_done_with_host_buffer != nullptr) {
        std::move(on_done_with_host_buffer)();
      }
      LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ28";
      on_done_with_host_buffer = [alloc]() { free(alloc); };
      LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ29";
    }

    LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ211";
    ...
    rpc_helper->host_buffer_store()
        ...
        .OnReady([on_done = std::move(on_done_with_host_buffer),
          ...
          LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer foofoo1";
          if (!s.ok()) {
          ...
          LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer foofoo2";
        });
  }
  LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ3";
  ...
  *req->mutable_shape() = shape.ToProto();
  ...
  LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer XYZ4";
        ...
        .OnReady(
            [host_buffer_handle](
                absl::StatusOr<std::shared_ptr<MakeArrayFromHostBufferResponse>>
                    resp) {
              LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer foofoo10";
              if (resp.ok()) {
              ...
              LOG(ERROR) << "madthanu: MakeArrayFromHostBuffer foofoo11";
            });
