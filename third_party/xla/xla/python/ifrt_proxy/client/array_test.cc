...
class ArrayTest : public ::testing::Test {
 protected:
  ...
  std::shared_ptr<xla::ifrt::MockClient> mock_client_;
  std::shared_ptr<xla::ifrt::MockDevice> mock_device_;
  std::shared_ptr<MockClientHostBufferStore> host_buffer_store_;
...
TEST_F(ArrayTest, FullyReplicatedShard) {
  ...
  ON_CALL(*mock_client_, GetDefaultLayout(_, _, _, _))
      .WillByDefault(Return(kLayout1));
...
TEST_F(ArrayTest, RemapArraysSuccess) {
  ...
  auto result = Array::RemapArrays(mock_client_.get(), rpc_helper_, plan,
