...
using DevicePutFunc = std::function<absl::StatusOr<LazyDevicePutResult>(
...
absl::StatusOr<LazyDevicePutResult> HandlePythonScalar(
...
absl::StatusOr<LazyDevicePutResult> HandlePythonInt(
    nb::handle obj, ifrt::Client* client, ifrt::Device* to_device,
    const DevicePutOptions& options, ifrt::MemoryKind to_memory_kind) {
  PrimitiveType type;
...
absl::StatusOr<LazyDevicePutResult> HandleNumpyScalar(
...
absl::StatusOr<LazyDevicePutResult> HandleNumpyArray(
    nb::handle h, ifrt::Client* client, ifrt::Device* to_device,
    const DevicePutOptions& options, ifrt::MemoryKind to_memory_kind) {
...
absl::StatusOr<LazyDevicePutResult> HandlePyArray(
    nb::handle obj, ifrt::Client* client, ifrt::Device* to_device,
    const DevicePutOptions& options, ifrt::MemoryKind to_memory_kind) {
...
absl::StatusOr<LazyDevicePutResult> LazyDevicePut(
    nb::handle arg, ifrt::Client* client, ifrt::Device* to_device,
    const DevicePutOptions& options, ifrt::MemoryKind to_memory_kind) {
...
absl::StatusOr<DevicePutResult> DevicePut(nanobind::handle arg,
                                          ...
                                          ifrt::MemoryKind to_memory_kind) {
  TF_ASSIGN_OR_RETURN(
      LazyDevicePutResult result,
      LazyDevicePut(arg, client, to_device, options, to_memory_kind));
  // Must release the GIL before calling IFRT because backends may decide to
  // block/sleep for device buffer allocation.
