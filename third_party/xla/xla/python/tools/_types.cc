...
py::object MakeNdarray(const xla::LiteralProto& proto) {
  ...
  auto nbobj = nb::google::ValueOrThrow(xla::LiteralToPython(std::move(lit)));
...
xla::PrimitiveType DtypeToEtype(const py::dtype& py_d) {
  ...
  return nb::google::ValueOrThrow(xla::DtypeToPrimitiveType(nb_d));
}

py::dtype EtypeToDtype(xla::PrimitiveType p) {
  auto nb_d = nb::google::ValueOrThrow(xla::PrimitiveTypeToNbDtype(p));
...
PYBIND11_MODULE(_types, py_m) {
  ...
  // We call `nb::google::ValueOrThrow` to make make the conversions explicit,
  // since `nb::detail::type_caster` disallows raising exceptions.
  nb::google::ImportStatusModule();
