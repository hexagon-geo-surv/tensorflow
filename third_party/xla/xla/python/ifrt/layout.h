...
class CompactLayout final : public llvm::RTTIExtends<CompactLayout, Layout> {
 public:
  ...
  // Creates a compact layout that represents a major-to-minor layout for a
  // shard shape with `num_shard_shape_dims` dimensions.
  static absl_nonnull std::unique_ptr<CompactLayout> CreateMajorToMinor(
      int num_shard_shape_dims);

  absl::Span<const int> permutation() const { return permutation_; }
 ...
 private:
  using Permutation = absl::InlinedVector<int, 6>;

  explicit CompactLayout(Permutation permutation)
