...
absl::StatusOr<std::string> PjRtExecutable::CommonMetadata::Serialize(
  ...
  *metadata.mutable_executable_version() = serialized_executable_version;
  ...
  if (outputs_bundle_slice_sizes.has_value()) {
    metadata.mutable_outputs_bundle_slice_sizes()->Reserve(
        outputs_bundle_slice_sizes->size());
    for (int size : *outputs_bundle_slice_sizes) {
      metadata.add_outputs_bundle_slice_sizes(size);
    }
  }
...
PjRtExecutable::CommonMetadata::Deserialize(
  ...
  if (metadata.outputs_bundle_slice_sizes_size() > 0) {
    outputs_bundle_slice_sizes.emplace();
    outputs_bundle_slice_sizes->reserve(
        metadata.outputs_bundle_slice_sizes_size());
    for (int size : metadata.outputs_bundle_slice_sizes()) {
      outputs_bundle_slice_sizes->push_back(size);
    }
  }
