...
AssembleStringArrayFromSingleDeviceStringArrays(
  ...
  // BufferHolder contains a `BasicStringArray::Buffer` along with per-shard
  // vectors of the underlying strings and string_views. Not thread safe.
  struct BufferHolder {
    explicit BufferHolder(int num_shards)
        : buffers(num_shards),
          per_shard_strings(num_shards),
          per_shard_string_views(num_shards) {}
    void clear() {
      buffers.clear();
      per_shard_strings.clear();
      per_shard_string_views.clear();
    }
    void CopyBuffer(absl::Span<const absl::string_view> strbuf,
                    int shard_index) {
      ...
      for (int i = 0; i < strbuf.size(); ++i) {
        LOG(ERROR) << "2DO: " << " string " << i
                   << "'s size: " << strbuf[i].size();
        strings.push_back(std::string(strbuf[i].data(), strbuf[i].size()));
      }
      for (const auto& str : strings) {
        views.push_back(str);
      }
      buffers[shard_index] = (absl::MakeSpan(views));
    }
    BasicStringArray::Buffers buffers;
  ...
  auto buffer_holder =
      std::make_shared<BufferHolder>(sharding->devices().size());
  auto on_done_with_buffer = [buffer_holder = buffer_holder]() {};

  struct BufferCopyingState {
    BufferCopyingState(int num_buffers_to_copy,
                       std::shared_ptr<BufferHolder> buffer_holder)
        : num_buffers_to_copy(num_buffers_to_copy),
          buffer_holder(std::move(buffer_holder)) {}
    absl::Mutex mu;
    int num_buffers_to_copy ABSL_GUARDED_BY(mu);
    std::shared_ptr<BufferHolder> buffer_holder ABSL_GUARDED_BY(mu);
  };
  auto buffer_copying_state = std::make_shared<BufferCopyingState>(
      arrays.size(), std::move(buffer_holder));
  ...
  auto buffer_copier = [state = buffer_copying_state,
    ...
    if (!strbuf.ok()) {
      ...
      state->buffer_holder->clear();
      return;
    }

    state->buffer_holder->CopyBuffer(strbuf->front(), shard_index);

    if (--state->num_buffers_to_copy > 0) {
      return;  // We have more single device arrays we need to wait for.
    }
    // We have all the buffers. Set the promise
    promise.Set(std::move(state->buffer_holder->buffers));
  };
  ...
  for (int i = 0; i < arrays.size(); ++i) {
    ...
    basic_string_array->buffers().OnReady(
        [shard_index = i, buffer_copier = buffer_copier](
          ...
          buffer_copier(strbuf, shard_index);
        });
  }
