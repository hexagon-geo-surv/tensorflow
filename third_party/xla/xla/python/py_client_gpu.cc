...
absl::Status XlaFfiPythonGpuCallbackImpl(
    gpuStreamHandle stream, ifrt::HostCallBackDescriptors* descriptors,
  ...
  for (size_t i = 0; i < arity; ++i) {
    ...
    auto dims = absl::Span<const int64_t>(arg->dimensions().begin(),
                                          arg->dimensions().size());
    auto array =
        nb_numpy_ndarray(nb_dtype(ifrt::PrimitiveTypeToString(dtype)), dims,
                         std::nullopt, const_cast<void*>(host_input_buffers[i]),
                         /*base=*/base);
    array.attr("flags").attr("writeable") = nb::bool_(false);
    PyTuple_SET_ITEM(host_input_arrays.ptr(), i, array.inc_ref().ptr());
  }
...
XLA_FFI_DEFINE_HANDLER(XlaFfiPythonGpuCallback, XlaFfiPythonGpuCallbackImpl,
                       ffi::Ffi::Bind()
