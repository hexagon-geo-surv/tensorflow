...
struct DummyRecordWriterBaseOld {
  enum class Padding {
    kFalse,
    kTrue,
    kInitially,
...
struct DummyOptionsOld {
  ...
  void set_pad_to_block_boundary(DummyRecordWriterBaseOld::Padding pad) {
    if (pad == DummyRecordWriterBaseOld::Padding::kTrue) {
...
template <typename T, typename = void>
struct TestHasPaddingEnum : std::false_type {};

template <typename T>
struct TestHasPaddingEnum<T, std::void_t<typename T::Padding>>
    : std::true_type {};

template <typename Options, typename RecordWriterBase, bool HasEnum>
struct TestPadToBlockBoundaryHelper {
  static void Call(Options& options) {}
};

template <typename Options, typename RecordWriterBase>
struct TestPadToBlockBoundaryHelper<Options, RecordWriterBase, true> {
  static void Call(Options& options) {
...
void TestSetPadding(Options& options) {
  if constexpr (TestHasSetPadding<Options>::value) {
    ...
    TestPadToBlockBoundaryHelper<
        Options, RecordWriterBase,
        TestHasPaddingEnum<RecordWriterBase>::value>::Call(options);
  }
}
...
TEST(SaveProfileTest, SetPaddingCompatibility) {
  ...
  struct DummyRWBNew {};
  TestSetPadding<DummyOptionsNew, DummyRWBNew>(new_opts);
  EXPECT_TRUE(new_opts.set_padding_called);

  DummyOptionsOld old_opts;
  TestSetPadding<DummyOptionsOld, DummyRecordWriterBaseOld>(old_opts);
