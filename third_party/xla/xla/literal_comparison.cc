...
class NearComparator {
 ...
 private:
  ...
  template <typename T>
  int CalculateFP8Distance(T expected, T actual) {
    switch (error_.low_precision_fp_error_spec.type) {
      case ErrorSpec::LowPrecisionFPType::kFP8E4M3FN:
        return fp8_util::CalculateF8Distance(tsl::float8_e4m3fn(expected),
                                             tsl::float8_e4m3fn(actual));
      case ErrorSpec::LowPrecisionFPType::kFP8E5M2:
        return fp8_util::CalculateF8Distance(tsl::float8_e5m2(expected),
                                             tsl::float8_e5m2(actual));
      default:
        LOG(WARNING) << "Comparing error for unsupported FP8 type.";
        return -1;
    }
  }
  ...
  void CompareValues(T expected, T actual, int64_t linear_index) {
    ...
    if (CompareEqual<T>(expected, actual, {linear_index})) {
      ...
      fp8_distance = CalculateFP8Distance<T>(expected, actual);
    ...
    bool is_within_n_ulps;
    bool is_fp8 = error_.low_precision_fp_error_spec.type !=
                  ErrorSpec::LowPrecisionFPType::kUnused;
    if (is_fp8 && error_.low_precision_fp_error_spec.within_n_f8_ulps >= 0) {
      is_within_n_ulps =
          fp8_distance <= error_.low_precision_fp_error_spec.within_n_f8_ulps;
    }

    const bool is_abs_mismatch =
        (is_fp8 && is_within_n_ulps) ? false : (abs_error > error_.abs);
    const bool is_rel_mismatch =
        (is_fp8 && is_within_n_ulps) ? false : (rel_error > error_.rel);
