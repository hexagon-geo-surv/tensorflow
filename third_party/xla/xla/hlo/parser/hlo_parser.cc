...
bool HloParserImpl::ParseAxisRef(
  ...
  if (!absl::SimpleAtoi(axis_name_or_index, &axis_index)) {
  ...
  if (axis_index == -1) {
    return TokenError(absl::StrCat("axis ", axis_name_or_index,
                                   " not found in mesh ", mesh.ToString()));
  }
...
bool HloParserImpl::ParseSingleSharding(
  ...
  if (lexer_.GetKind() == TokKind::kw_unreduced) {
    ...
    // TODO: Create HloSharding::Unreduced() helper.
  ...
  if (lexer_.GetKind() == TokKind::kw_manual) {
    ...
    for (int i = 0; i < mesh->num_axes(); ++i) {
      manual_axes.push_back(AxisRef(i));
    }
    sharding = NamedSharding(*mesh, {}, {}, {}, manual_axes, metadata);
    return ParseToken(TokKind::kRbrace,
                      "expected '}' to end sharding attribute");
  }
