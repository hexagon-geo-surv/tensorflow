...
class HloParserImpl : public HloParser {
 ...
 private:
  ...
  HloComputation* CreateAsyncWrappedComputation(
      HloOpcode async_wrapped_opcode, const Shape& shape,
...
HloInstruction* HloParserImpl::CreateInstruction(  // NOLINT
  ...
  switch (opcode) {
    ...
    case HloOpcode::kAsyncDone: {
      ...
      if (operand_shape.IsTuple() && operand_shape.tuple_shapes().size() >= 2 &&
          operand_shape.tuple_shapes(0).IsTuple()) {
        UpdateAsyncWrappedComputation(
...
HloComputation* HloParserImpl::CreateAsyncWrappedComputation(
  ...
  // For late-bound async operations, some operands might be missing when
  // parsing the start operation (to be provided later in update operations).
  // Since some opcodes (like kDot) have a fixed arity and require all operands
  // during instruction creation (e.g., for shape inference), we ensure the
  // wrapped computation has at least as many parameters as the opcode's arity
  // by adding dummy parameters. These dummies will be updated with actual
  // parameters later in UpdateAsyncWrappedComputation when the operands are
  // bound.
  ...
  for (int i = 0; i < num_async_operands; ++i) {
    // Use F32[1] as a default shape for dummy parameters. The shape will be
    // updated later in UpdateAsyncWrappedComputation when the actual operands
    // are bound. F32[1] is chosen as a small, valid array shape.
    Shape param_shape = i < operand_shapes.size()
                            ? operand_shapes[i]
  ...
  if (root->opcode() == HloOpcode::kCall && root->has_to_apply()) {
    HloComputation* to_apply = root->to_apply();
    *root->mutable_shape() = to_apply->root_instruction()->shape();
    if (operand_shapes.size() < to_apply->num_parameters()) {
      for (int i = operand_shapes.size(); i < to_apply->num_parameters(); ++i) {
        auto dummy_param = HloInstruction::CreateParameter(
            i, to_apply->parameter_instruction(i)->shape(),
            "async_param_dummy");
        auto status_or_param =
            async_wrapped_builder.AddParameter(std::move(dummy_param));
        if (!status_or_param.ok()) {
          return nullptr;
        }
        root->AppendOperand(status_or_param.value());
      }
    }
  }
