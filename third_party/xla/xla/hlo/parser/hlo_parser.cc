...
class HloParserImpl : public HloParser {
 ...
 private:
  ...
  bool ParseShape(Shape* result, bool never_fallback_to_default_layout = false);
...
bool HloParserImpl::ParseComputationLayout(
  ...
  while (lexer_.GetKind() != TokKind::kRparen) {
    Shape param;
    if (!ParseShape(&param, options_.keep_module_auto_layouts())) {
      return false;
    }
  ...
  Shape result;
  if (!ParseShape(&result, options_.keep_module_auto_layouts())) {
    return false;
  }
...
bool HloParserImpl::ParseShape(Shape* result,
                               bool never_fallback_to_default_layout) {
  if (EatIfPresent(TokKind::kLparen)) {  // Tuple
    ...
    if (lexer_.GetKind() == TokKind::kRparen) {
      ...
      do {
        shapes.emplace_back();
        if (!ParseShape(&shapes.back(), never_fallback_to_default_layout)) {
          return false;
        }
      } while (EatIfPresent(TokKind::kComma));
    }
  ...
  if (!never_fallback_to_default_layout &&
      (options_.fill_missing_layouts() || ShapeUtil::IsScalar(*result))) {
