...
std::vector<std::vector<std::string>> NamedSharding::JaxPartitionsSpec() const {
  std::vector<std::vector<std::string>> partitions(dim_shardings_.size());
  for (int i = 0; i < dim_shardings_.size(); ++i) {
    partitions[i].resize(dim_shardings_[i].axes().size());
    CHECK(dim_shardings_[i].is_closed())
        << "jax_partitions_spec should only be "
           "called for closed dimension shardings.";
    for (int j = 0; j < dim_shardings_[i].axes().size(); ++j) {
      const AxisRef& axis = dim_shardings_[i].axes()[j];
      partitions[i][j] = mesh_.axis_names()[axis.mesh_axis_index()];
      CHECK(!axis.sub_axis_info().has_value())
          << "jax_partitions_spec should only be called for non-sub-axis.";
    }
  }
  return partitions;
}
