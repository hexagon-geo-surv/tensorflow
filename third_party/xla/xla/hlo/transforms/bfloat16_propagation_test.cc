...
// Tests that a non-associative scan is NOT touched by the new scan
// propagation logic. Non-associative scans get expanded to while loops by
// ScanExpander before reaching any backend lowering, so applying the scan
// dispatch here would conflict with the kWhile path that runs after
// expansion. Verify that the body of a non-associative scan retains its
// original F32 precision and structure.
TEST_F(BFloat16PropagationTest, NonAssociativeScanNotPropagated) {
  constexpr absl::string_view kHlo = R"(
...
ENTRY main {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(kHlo));
  // Propagation may still change downstream ops (slice/reshape feeding the
  // dot), but the scan body itself must remain F32 with no duplicator.
  ...
  HloInstruction* body_root = scan->to_apply()->root_instruction();
  ASSERT_EQ(body_root->opcode(), HloOpcode::kTuple);
  // Both slots stay F32: the new scan logic is gated on is_associative ==
  // TRI_STATE_TRUE.
  for (const Shape& s : body_root->shape().tuple_shapes()) {
    EXPECT_EQ(s.element_type(), F32);
  }
  // Both slots remain backed by the same `add` (no duplicator inserted).
  EXPECT_EQ(body_root->operand(0), body_root->operand(1));
}
