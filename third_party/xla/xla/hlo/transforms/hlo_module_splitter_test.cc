...
using HloModuleSplitterTest = HloHardwareIndependentTestBase;
...
CHECK:   ROOT {{.*}} add(
  ...
  ASSERT_OK_AND_ASSIGN(bool sub_filecheck_ok,
                       RunFileCheck(splitter.submodules()[0]->ToString(),
                                    expected_submodule_hlo));
...
CHECK:   ROOT {{.*}} custom-call({{.*}}), custom_call_target="_xla_multi_module_call",{{.*}}backend_config="callee"
  ...
  TF_ASSERT_OK_AND_ASSIGN(bool filecheck_ok,
                          RunFileCheck(module->ToString(), expected_hlo));
  EXPECT_TRUE(filecheck_ok);
}
