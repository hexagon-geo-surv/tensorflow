...
absl::StatusOr<bool> ReduceWindowRewriter::TryOptimizeAssociativeScan(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      RewriteScanAsTreeReduction(parent, sources, inits, scan->to_apply(),
                                 scan->shape(), rank, scan_dim, scan_length,
                                 /*forward_scan=*/!scan->is_reverse(),
                                 /*is_exclusive=*/false));
  TF_RETURN_IF_ERROR(scan->ReplaceAllUsesWith(result));
