...
struct Chunk {
  int64_t start_dim;
  int64_t size;
  int64_t result_dim_idx;  // The dimension index in the outer reshape
                           // output
};
...
absl::StatusOr<bool> AlgebraicSimplifierVisitor::TryHoistTransposeOfReshape(
    HloInstruction* reshape) {
  if (reshape->mutable_operand(0)->opcode() != HloOpcode::kTranspose) {
    return false;
  }
  auto transpose = reshape->mutable_operand(0);
  if (transpose->operand(0)->opcode() != HloOpcode::kReshape) {
    return false;
  }
