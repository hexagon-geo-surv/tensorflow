...
class TuplePointsToAnalysis : public DfsHloVisitorWithDefault {
 ...
 private:
  ...
  // instruction's computation-local id. Both id spaces are small and dense,
  // so this replaces a large hash map keyed by instruction unique_id (which
  // was a top find() hotspot on big modules) with two array indexes.
