...
bool ShardingIsConsistent(const HloSharding& partial_sharding,
  ...
  for (size_t i = 0; i < partial_sharding.tile_assignment().num_dimensions();
       ++i) {
    if (strict && partial_sharding.tile_assignment_dim(i) > 1 &&
        partial_sharding.tile_assignment_dim(i) ==
            complete_sharding.tile_assignment_dim(i)) {
      return true;
    }
    if (!strict && partial_sharding.tile_assignment_dim(i) > 1 &&
        complete_sharding.tile_assignment_dim(i) > 1) {
      return true;
    }
  }
  return false;
