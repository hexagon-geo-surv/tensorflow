...
LogicalResult ExportXlaOp(WhileOp op, OpLoweringContext ctx) {
  ...
  if (has_any_original_value) {
    ...
    // The implicit results are the same as the implicit operands, which are
    // unchanged in the while loop, so the original value of the implicit
    // results are the same as the original value of the implicit operands.
    for (auto [implicit_index, implicit_operand] :
         llvm::enumerate(implicit_operands)) {
