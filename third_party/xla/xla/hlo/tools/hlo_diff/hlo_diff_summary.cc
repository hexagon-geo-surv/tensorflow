...
std::unique_ptr<const DiffSummary> ConstructDiffSummary(
  ...
  for (const auto [left, right] : diff_result.unchanged_instructions) {
    if (left->has_called_computations()) {
      for (const HloComputation* left_computation :
           left->called_computations()) {
        const auto it = summary->computation_summary.find(left_computation);
        if (it != summary->computation_summary.end() &&
            !it->second.all_unchanged) {
          LOG(INFO) << "left: " << left->name();
          LOG(INFO) << "- left_computation: " << left_computation->name();
          LOG(INFO) << "- - all_unchanged: " << it->second.all_unchanged;
        }
      }
    }
  }
