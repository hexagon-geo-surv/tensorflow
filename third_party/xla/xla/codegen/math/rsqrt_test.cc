...
TEST(RsqrtTest, EmitRsqrtF64_EdgeCasesHasAvx) {
  if (hasAvx512Support()) {
    ...
    EXPECT_THAT(rsqrt(large), NearUlps<double>(one_over_sqrt(large), 1));
    double large2 = 6.112156648698989e+307;
    EXPECT_THAT(rsqrt(large2), NearUlps<double>(one_over_sqrt(large2), 1));
  }
}
...
void TestRsqrtF64EdgeCases() {
  if (hasAvx512Support()) {
    ...
    for (size_t i = 0; i < kN; ++i) {
      EXPECT_THAT(actual_tiny[i], NearUlps<NativeType>(expected_tiny[i], 1))
    ...
    for (size_t i = 0; i < kN; ++i) {
      EXPECT_THAT(actual_edge[i], NearUlps<NativeType>(expected_edge[i], 1))
