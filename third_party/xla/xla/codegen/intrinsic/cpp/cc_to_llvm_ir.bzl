...
def _get_header_category(path):
    ...
    if any([x in path for x in ["libcxx/include", "libc++/include", "c++/"]]):
        return "libcxx"
...
def _process_toolchain_flags(flags, categories, src_path):
    ...
    for i, arg in enumerate(flags):
        ...
        # Handle split includes (e.g. ["-isystem", "path"])
        current_args = [arg]
        path = arg
...
def _cc_ir_header_impl(ctx):
    ...
    # Access compiler_files, filtering out linker artifacts and startup libs.
    compiler_files = ctx.attr._cc_toolchain[cc_common.CcToolchainInfo].all_files
    filtered_compiler_files = depset([
        f
        for f in compiler_files.to_list()
    ...
    additional_inputs = []
    if hasattr(cc_toolchain, "libc_top") and cc_toolchain.libc_top:
        additional_inputs.append(cc_toolchain.libc_top)
    if hasattr(cc_toolchain, "static_runtime_libs") and cc_toolchain.static_runtime_libs:
        additional_inputs.append(cc_toolchain.static_runtime_libs)
