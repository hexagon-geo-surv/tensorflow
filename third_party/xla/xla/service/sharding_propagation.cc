    ...
    case HloOpcode::kGather: {
      ...
      if (is_spmd_) {
        ...
        if (hlo_sharding_util::IsSpatiallyPartitioned(
                instruction->operand(0))) {
          std::vector<int64_t> operand_parallel_dims(
              dnums.operand_batching_dims().begin(),
              dnums.operand_batching_dims().end());
          if (gather_parallel_dims) {
            operand_parallel_dims.insert(
                operand_parallel_dims.end(),
                gather_parallel_dims->operand_parallel_dims.begin(),
                gather_parallel_dims->operand_parallel_dims.end());
          }
