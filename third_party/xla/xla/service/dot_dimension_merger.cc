 ...
 public:
  Status HandleDot(HloInstruction* dot) override {
    ...
    for (SparsityDescriptor descriptor :
      ...
      int skip_batch_dims = absl::c_count_if(
          descriptor.index() == 0 ? dnums.lhs_batch_dimensions()
                                  : dnums.rhs_batch_dimensions(),
          [&](int64_t batch_dim) {
            return batch_dim < descriptor.dimension();
          });
      if (skip_batch_dims > 0) {
        --skip_batch_dims;  // First batch dim is kept in place.
      }
      descriptor.set_dimension(descriptor.dimension() - skip_batch_dims);
