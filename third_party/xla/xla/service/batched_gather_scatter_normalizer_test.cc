...
TEST_F(BatchedGatherScatterNormalizerTest,
       NormalizeBatchGatherIndicesRemainsSorted) {
  constexpr absl::string_view kModuleStr = R"(
...
TEST_F(BatchedGatherScatterNormalizerTest,
       NormalizeBatchGatherIndicesRemainsUnsorted) {
  constexpr absl::string_view kModuleStr = R"(
HloModule StringifyGather, entry_computation_layout={(f32[2,3,4,512]{3,2,1,0}, s64[2,3,1]{2,1,0})->f32[2,3,5]{2,1,0}}
...
TEST_F(BatchedGatherScatterNormalizerTest,
       NormalizeBatchScatterIndicesRemainsSorted) {
  constexpr absl::string_view kModuleStr = R"(
...
TEST_F(BatchedGatherScatterNormalizerTest,
  ...
  constexpr absl::string_view kModuleStr = R"(
HloModule StringifyGather, entry_computation_layout={(f32[2,2147483648,512]{2,1,0}, s8[2,2147483648,1]{2,1,0})->f32[2,2147483648,5]{2,1,0}}

ENTRY %Gather (input_tensor: f32[2,2147483648,512], start_indices: s8[2,2147483648,1]) -> f32[2,2147483648,5] {
  %input_tensor = f32[2,2147483648,512]{2,1,0} parameter(0)
  %start_indices = s8[2,2147483648,1]{2,1,0} parameter(1)
  ROOT %gather = f32[2,2147483648,5]{2,1,0}
    gather(f32[2,2147483648,512]{2,1,0} %input_tensor, s8[2,2147483648,1]{2,1,0} %start_indices),
    offset_dims={2}, collapsed_slice_dims={}, start_index_map={2}, operand_batching_dims={0,1},
    start_indices_batching_dims={0,1}, index_vector_dim=2, slice_sizes={1,1,5}
})";

  RunAndFilecheckHloRewrite(kModuleStr, BatchedGatherScatterNormalizer(), R"(
         CHECK: %[[IOTA1:.*]] = s64[2,2147483648,1]{{.*}} iota(), iota_dimension=0
         CHECK: %[[IOTA2:.*]] = s64[2,2147483648,1]{{.*}} iota(), iota_dimension=1
         CHECK: %[[CONVERT:.*]] = s64[2,2147483648,1]{{.*}} convert(%start_indices)
         CHECK: %[[INDICES_CONCAT:.*]] = s64[2,2147483648,3]{{.*}} concatenate(%[[IOTA1]], %[[IOTA2]], %[[CONVERT]])
         CHECK: ROOT %[[GATHER:.*]] = f32[2,2147483648,5]{{.*}} gather(
    CHECK-SAME:     %input_tensor, %[[INDICES_CONCAT]]),
    CHECK-SAME:     offset_dims={2},
    CHECK-SAME:     collapsed_slice_dims={0,1},
    CHECK-SAME:     start_index_map={0,1,2},
