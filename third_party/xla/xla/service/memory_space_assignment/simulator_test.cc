...
TEST_F(MemorySpaceAssignmentSimulatorTest, SingleLayerNestedLoop) {
  ...
  EXPECT_EQ(runtime_simulator_->SimulateElapsedTime(
                module_.get(), *hlo_live_range_, allocations_),
            84);
}
...
TEST_F(MemorySpaceAssignmentSimulatorTest, AsyncCopyWithComputationOverhead) {
  ...
  // Workflow:
  // neg_compute: | 2 sec (memory-bound)  |
...
TEST_F(MemorySpaceAssignmentSimulatorTest, SingleAsyncSliceCopyOverhead) {
  absl::string_view hlo_string =
      R"(HloModule module, is_scheduled=true
        async_wrapped {
          async_param = f32[3072,2048] parameter(0)
          ROOT slice = f32[768,2048]{1,0:S(1)} slice(f32[3072,2048] async_param), slice={[1536:2304], [0:2048]}
        }
        ENTRY Entry {
...
TEST_F(SimulateAsyncCopyDoneTest,
       ProcessAsyncCopiesInTimeWindowWithSharedBandwidth) {
  absl::string_view hlo_string =
      R"(HloModule module, is_scheduled=true
      ENTRY Entry {
        param_0 = f32[128] parameter(0)
  ...
  // Both queues are not empty, so the bandwidth
  // is shared. Each of the request at the front of the queue process 100 sec *
  // 0.5 bytes/sec = 50 bytes.
  float time_window = 100;
  runtime_simulator_->ProcessAsyncCopiesInTimeWindow(time_window);

  EXPECT_THAT(runtime_simulator_->GetOutstandingReadDefaultQueue(),
              ElementsAreArray({memory_space_assignment::OutstandingAsyncCopy{
                  copy_start_1_inst, 462}}));

  EXPECT_THAT(runtime_simulator_->GetOutstandingWriteDefaultQueue(),
              ElementsAreArray({memory_space_assignment::OutstandingAsyncCopy{
                  copy_start_2_inst, 78}}));
}

TEST_F(SimulateAsyncCopyDoneTest,
       ProcessAsyncCopiesInTimeWindowWithFullBandwidth) {
  absl::string_view hlo_string =
      R"(HloModule module, is_scheduled=true
      ENTRY Entry {
        param_0 = f32[128] parameter(0)
  ...
  // Only the read queue is not empty, so it can use the full bandwidth.
  // In the time window, it can process 100 sec * 1 bytes/sec = 100 bytes.
  float time_window = 100;
  runtime_simulator_->ProcessAsyncCopiesInTimeWindow(time_window);

  EXPECT_THAT(runtime_simulator_->GetOutstandingReadDefaultQueue(),
              ElementsAreArray({memory_space_assignment::OutstandingAsyncCopy{
                  copy_start_1_inst, 412}}));
  EXPECT_THAT(runtime_simulator_->GetOutstandingWriteDefaultQueue(), IsEmpty());
}

TEST_F(SimulateAsyncCopyDoneTest, ProcessAsyncCopyInTimeWindowWithEmptyQueue) {
  absl::string_view hlo_string =
      R"(HloModule module, is_scheduled=true
      ENTRY Entry {
        ROOT param_0 = f32[128] parameter(0)
      }
    )";
