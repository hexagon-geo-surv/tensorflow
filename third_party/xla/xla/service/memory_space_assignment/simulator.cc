...
MemoryTransferDirection GetAsyncCopyDirection(
  ...
  CHECK(async_copy_start->opcode() == HloOpcode::kCopyStart ||
        (async_copy_start->opcode() == HloOpcode::kAsyncStart &&
         async_copy_start->async_wrapped_instruction()->opcode() ==
             HloOpcode::kSlice));
...
float RuntimeSimulator::SimulateComputeInstruction(
    ...
    absl::Span<const std::pair<int64_t, ShapeIndex>>
        operands_in_alternate_memory,
    absl::Span<const ShapeIndex> outputs_in_alternate_memory) {
  // Calculate the time window in which the instruction does not access the
  // default memory.
  float default_memory_idle_time =
      cost_analysis_->GetDefaultMemoryBandwidthIdleTime(
          *instruction, operands_in_alternate_memory,
          outputs_in_alternate_memory);
  // Execute the outstanding async copy in the time window.
  ProcessAsyncCopiesInTimeWindow(default_memory_idle_time);
...
void RuntimeSimulator::ProcessAsyncCopiesInTimeWindow(float time_window) {
  if (time_window <= 0) {
    return;
  }
  float elapsed_time_window = time_window;
  // This loop simulates the execution of the front memory requests in the
  // read and/or write queues. The loop terminates when the time window is
  ...
  while ((!outstanding_read_default_queue_.empty() ||
    ...
    float bytes_to_process = available_bandwidth * elapsed_time_window;
    if (!outstanding_read_default_queue_.empty()) {
      bytes_to_process = std::min(
          bytes_to_process,
    ...
    elapsed_time_window -= real_elapsed_time;
...
float RuntimeSimulator::SimulateElapsedTime(
  ...
  if (cost_analysis_->base_costs().BytesPerSecond() <= 0.0) {
    return 0.0;
  }
  ...
  for (const HloComputation* computation :
    ...
    // We include the overhead of the async copy instructions when computing the
    // runtime of the caller (copy-done, slice). Thus, we can skip the async
    // computation as callee.
    ...
    for (const HloInstruction* instruction :
      ...
      if (instruction->opcode() == HloOpcode::kCopyStart ||
        ...
        inst_elapsed = SimulateComputeInstruction(instruction,
                                                  operands_in_alternate_memory,
                                                  outputs_in_alternate_memory);
      }
      if (inst_elapsed > 0) {
