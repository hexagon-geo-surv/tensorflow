...
bool MsaAlgorithm::IsReplaceableSyncCopyCandidate(
  ...
  if (instruction->shape().layout().memory_space() != 0 ||
...
MsaAlgorithm::GetJointProcessedValuesForSyncCopyReplacement(
  ...
  // Adds the HloValues that are related to a given instruction to the worklist
  auto add_to_worklist = [&](const HloInstruction* inst) {
    ...
    // auto& hlo_values =
    //     alias_analysis_.dataflow_analysis().GetValueSet(inst).values();
...
void MsaAlgorithm::CreateAllocationValuesForJointProcessedIntervals(
    const std::vector<const HloValue*>& joint_processed_values,
    std::vector<AllocationValue>& joint_processed_allocation_values,
    std::vector<std::vector<const MsaBufferInterval*>>&
        joint_processed_colocated_intervals) {
  for (auto& interval_hlo : joint_processed_values) {
    ...
    CreateAllocationValuesFromColocatedIntervals(
        colocated_intervals, joint_processed_allocation_values);
    joint_processed_colocated_intervals.push_back(colocated_intervals);
  }
  if (VLOG_IS_ON(3)) {
    VLOG(3) << "Joint-processed allocation values: ";
    if (joint_processed_allocation_values.empty()) {
      VLOG(3) << "  --Empty--";
    }
    for (int i = 0; i < joint_processed_allocation_values.size(); i++) {
      VLOG(3) << "  " << i + 1 << "/"
              << joint_processed_allocation_values.size() << ") "
              << joint_processed_allocation_values.at(i).ToShortString();
    }
  }
}
