...
// bitcasts and reduces to return the dynamic-slice that moves the data from the
// host to the device. If no dynamic-slice is found, returns an empty optional.
std::optional<HloInstruction*> FindDSFromAnnotation(HloInstruction* hlo) {
  CHECK(hlo->IsCustomCall(kMoveToDeviceCustomCallTarget))
