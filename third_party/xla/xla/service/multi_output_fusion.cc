...
void MultiOutputFusion::UpdateBeforeFuse(HloInstruction* fusion,
  ...
  for (HloInstruction* use : fusion->users()) {
    bool inserted = candidates_index_.insert({use, candidates_.size()}).second;
    if (inserted) {
      candidates_.emplace_back(use);
    }
  }
  update_worklist_.clear();
  indices_to_update_.clear();
  reachability_->UpdateMultipleInstructions(fused, fusion, update_worklist_,
                                            indices_to_update_);
}
