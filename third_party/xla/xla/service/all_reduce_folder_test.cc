...
class AllReduceFolderTest : public HloTestBase {
 public:
  std::unique_ptr<HloPassInterface> CreateHloTranformationPass() override {
    return std::make_unique<AllReduceFolder>();
  }
};
...
TEST_F(AllReduceFolderTest, Simple) {
  TF_ASSERT_OK_AND_ASSIGN(
      auto module, TransformTrue(k2AllReduce, {{"$group_0", "{{0,1},{2,3}}"},
                                               {"$group_1", "{{0,2},{1,3}}"}}));
...
TEST_F(AllReduceFolderTest, SimpleSwap) {
  TF_ASSERT_OK_AND_ASSIGN(
      auto module, TransformTrue(k2AllReduce, {{"$group_1", "{{0,1},{2,3}}"},
                                               {"$group_0", "{{0,2},{1,3}}"}}));
  ExpectOneAllReduce(module.get(), "replica_groups={{0,1,2,3}}");
}
...
TEST_F(AllReduceFolderTest, BothEmptyReplicaGroups_NotTransformed) {
  TF_ASSERT_OK(
      TransformFalse(k2AllReduce, {{"$group_0", "{}"}, {"$group_1", "{}"}}));
}

TEST_F(AllReduceFolderTest, EmptyReplicaGroups_NotTransformed) {
  TF_ASSERT_OK(TransformFalse(
      k2AllReduce, {{"$group_0", "{}"}, {"$group_1", "{{0,2},{1,3}}"}}));
}
...
TEST_F(AllReduceFolderTest, MismatchOtherProperties0_NotTransformed) {
  ...
  TF_ASSERT_OK(TransformFalse(hlo_string));
}
...
TEST_F(AllReduceFolderTest, Foldable0) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto module, TransformTrue(hlo_string));
