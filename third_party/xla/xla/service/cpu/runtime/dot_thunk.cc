...
// Dot operation implemented as a matrix-matrix multiply (row-major x rowm-ajor
// or col-major x col-major). For batched dot operations, it is implemented as
// multiple matrix multiplications repeated for each batch element.
//
...
static MatMulDims GetMatMulDims(
  ...
  CHECK_GE(lhs_contracting_dims.size(), 0);
  CHECK_GE(rhs_contracting_dims.size(), 0);
  ...
  switch (element_type) {
    ...
    default:
      return Unimplemented(
          "Unsupported elemtn type for DotThunk::Execute: %s",
