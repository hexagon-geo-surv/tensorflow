...
class IrEmitter2 {
 ...
 private:
  ...
  // Emits LLVM IR that computes parallel partition bounds from the call frame
  ...
  // Emits LLVM IR using elemental loop emitter and given element generator. If
  // instruction is parallelized it will emit a parallel loop partition and
  // return the requested number of execution threads.
