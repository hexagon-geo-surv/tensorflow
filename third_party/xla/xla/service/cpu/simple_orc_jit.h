 ...
 public:
  ...
  SimpleOrcJIT(
      ...
      absl::AnyInvocable<void(const llvm::object::ObjectFile&)>
          post_codegen_hook,
      size_t num_jit_dylibs = 1, std::string max_cpu_isa = "");
  ...
  static std::unique_ptr<llvm::TargetMachine> InferTargetMachineForJIT(
      const llvm::TargetOptions& target_options,
      llvm::CodeGenOptLevel opt_level, std::string max_cpu_isa = "");
...
std::vector<std::string> DetectMachineAttributes(
    std::optional<tsl::port::CPUFeature> max_feature, bool& features_filtered);
