...
IrEmitter2::KernelThreadDims IrEmitter2::EmitKernelThreadDims(
  ...
  auto* gep = b.CreateStructGEP(call_frame_ty_, call_frame, 0, "tdims_gep");
  auto* tdims = b.CreateLoad(b.getPtrTy(), gep, "tdims");
...
IrEmitter2::KernelThread IrEmitter2::EmitKernelThread(llvm::IRBuilder<>& b,
  ...
  auto* gep = b.CreateStructGEP(call_frame_ty_, call_frame, 1, "tid_gep");
  auto* tids = b.CreateLoad(b.getPtrTy(), gep, "tids");
...
std::optional<IrEmitter2::ParallelConfig> IrEmitter2::GetParallelConfig(
    const HloInstruction* instr) {
  // Check if the instruction marked for parallel execution.
...
IrEmitter2::ParallelPartitionBounds IrEmitter2::EmitParallelPartitionBounds(
  ...
  // this array using block and thread dimension index passed in a call frame.
  //
...
absl::StatusOr<se::ThreadDim> IrEmitter2::EmitElementalLoops(
  ...
  // Emit a loop for a whole instruction.
