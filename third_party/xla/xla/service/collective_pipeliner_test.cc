...
while_body {
  ...
  dynamic-update-slice.35 = bf16[3,8,128] dynamic-update-slice(constant.395, ar.1.annotated, select.1348, constant.2561, constant.2561), frontend_attributes={_can_always_sink="1"}
  ROOT tuple = (s32[], bf16[3,8,128], bf16[3,8,128]) tuple(add.230, dynamic-update-slice.35, get-tuple-element.5)
}
...
ENTRY entry {
  ...
  // In ForwardSink, the sinked operations are placed in the ENTRY computation,
  // after the while loop. Wait, no, ForwardSink moves operations from inside
  // the loop to the end of the loop, BUT it actually applies them to the output
  // of the while loop! Let's find the LayoutConstraint custom call.
