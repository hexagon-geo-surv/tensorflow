...
// If the updated start index map is sorted, then the `indices_are_sorted`
// property is preserved, since each concatenated iota is monotonically
// increasing, and a sorted index map means the order of the index vector dim
// corresponds to the order of operand dims.
bool indicesAreSorted(bool indices_sorted_before,
                      absl::Span<const int64_t> updated_index_map) {
  return indices_sorted_before && absl::c_is_sorted(updated_index_map);
}
...
absl::StatusOr<HloInstruction*> NormalizeBatchGather(
  ...
  return gather->AddInstruction(HloInstruction::CreateGather(
      ...
      indicesAreSorted(gather->indices_are_sorted(), start_index_map)));
}
...
absl::StatusOr<HloInstruction*> NormalizeBatchScatter(
  ...
  return scatter->AddInstruction(HloInstruction::CreateScatter(
      ...
      indicesAreSorted(scatter->indices_are_sorted(),
