...
TEST_F(HloVerifierTest, SingleUserExcepetionForWrappedSendRecv) {
  const char* const kModuleStr = R"(
  wrapped_send {
    data = f32[] parameter(0)
    recv-start = token[] parameter(1)
    ROOT send = (f32[], u32[], token[]) send(data, recv-start), channel_id=1,
      frontend_attributes={_xla_send_recv_source_target_pairs={{0,1}}}
  }
  wrapped_recv {
    recv-start = token[] parameter(0)
    ROOT recv = (f32[], u32[], token[]) recv(recv-start), channel_id=1,
      frontend_attributes={_xla_send_recv_source_target_pairs={{1,0}}}
  }
  ENTRY main () -> f32[] {
    data = f32[] constant(5)
    recv-start = token[] after-all()
    async-recv-start = ((token[]), (f32[], u32[], token[]), s32[]) async-start(recv-start), calls=wrapped_recv
    async-send-start = ((f32[], token[]), (f32[], u32[], token[]), s32[]) async-start(data, recv-start), calls=wrapped_send
