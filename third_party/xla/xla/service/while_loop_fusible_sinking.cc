...
// Matches the while loop condition root instruction to find the index of the
// loop iteration variable.
int64_t GetLoopIterationVariableIndex(HloInstruction* while_instr) {
  HloComputation* while_cond = while_instr->while_condition();
  // For now, we only support the case where the while loop condition is a
  // comparison between the loop iteration variable gte and any hlo.
  if (!Match(while_cond->root_instruction(),
             match::Compare(match::GetTupleElement(), match::Op()))) {
    return -1;
  }
  return while_cond->root_instruction()->mutable_operand(0)->tuple_index();
}

// Rewrites broadcast(constant) that is passed to a while loop as an
// AllocateBuffer only if the buffer at that location is written to entirely in
// the loop body. This is done by creating a select between the initial value
...
absl::StatusOr<bool> TryRewritingBroadcastAsAllocateBuffer(
    HloInstruction* while_instr) {
  int64_t loop_iteration_variable_index =
      GetLoopIterationVariableIndex(while_instr);
  if (loop_iteration_variable_index == -1) {
    return false;
  }
  ...
  for (int64_t loop_index : loop_indices) {
    VLOG(3) << "Trying to sink " << while_instr->ToString() << " at index "
            << loop_index;
    ...
    // iterations. The select is guarded by a predicate that checks if the loop
    // iteration variable is equal to the first iteration value.
    HloInstruction* iteration_var_gte = hlo_query::GetUniqueGteInstruction(
        while_body->parameter_instruction(0), loop_iteration_variable_index);
    if (iteration_var_gte == nullptr) {
      return false;
    }
    HloInstruction* loop_iteration_variable_initial_value =
        while_instr->while_init()->mutable_operand(
            loop_iteration_variable_index);
    TF_ASSIGN_OR_RETURN(
    ...
    // Replace the use of the buffer in the body with the select.
...
absl::StatusOr<bool> WhileLoopFusibleSinking::Run(
  ...
  VLOG(5) << "Before WhileLoopFusibleSinking:";
