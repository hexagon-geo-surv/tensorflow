...
xla::Future<std::unique_ptr<CollectiveKernelThunk>> EmitCollectiveKernelThunk(
  ...
  using AllReduceStrategy = se::gpu::AllReduceStrategy;
  static constexpr bool kIsMultimemEnabled = false;
  const bool is_2shot = GetAllReduceStrategy(size_bytes, kIsMultimemEnabled) ==
                        AllReduceStrategy::kTwoShot;
  if (is_collective_kernel_enabled && has_rank_higher_than_1 && is_2shot) {
    RETURN_IF_ERROR(
        FlattenCollectiveComputation(fused_module->entry_computation()));
  }
      ...
      [thunk_info = std::move(thunk_info), buffers = std::move(buffers), config,
       is_async = !IsGPUSyncCollective(*instr), is_collective_kernel_enabled](
        ...
        return std::make_unique<CollectiveKernelThunk>(
            ...
            launch_dimensions, shmem_bytes, kIsMultimemEnabled,
