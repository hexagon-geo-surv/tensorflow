...
void GpuCollectiveBufferAnalysis(
  ...
  for (const HloBuffer& buffer : alias_analysis.buffers()) {
    ...
    VLOG(5) << "====================================";
    VLOG(5) << "HloBuffer: " << buffer.ToString();
    for (const HloValue* v : entry_input_values) {
      VLOG(5) << "  entry_input_value: " << v->ToShortString();
    }
    for (const HloValue* v : live_out_values) {
      VLOG(5) << "  live_out_value: " << v->ToShortString();
    }
    VLOG(5) << "  used_by_collective: " << used_by_collective;
    VLOG(5) << "  defined_by_collective: " << defined_by_collective;
...
absl::Status RunPostSchedulingCopyInsertion(HloModule* module,
  ...
  VLOG(5) << "==== HLO before AddSpecialCaseCopies ====";
