...
bool IsAlwaysSatisfied(mlir::AffineExpr expr, Interval interval) {
...
bool IsUnsatisfiable(mlir::AffineExpr expr, Interval interval) {
...
SimplifyConjointConstraints(const ConjointConstraints& conjunction) {
  ...
  for (const auto& [expr, interval] : conjunction) {
    if (IsAlwaysSatisfied(expr, interval)) {
      continue;
    }
    if (IsUnsatisfiable(expr, interval)) {
