...
ENTRY main {
  ...
  EXPECT_EQ(result->flops,
            6 * 96 * 128 * 1                     // ==> %concatenate
                + 86 * 96 * 128 * (32.0 / 96.0)  // ==> %exp
  );
  // The bytes read contribution for a single instruction is calculated as:
  // element_type_size * num_blocks * tile_size * multiplier
  EXPECT_EQ(result->bytes_read,
            4 * 96 * 128 * (64.0 / 96.0)        // ==> %p1
                + 4 * 96 * 128 * (32.0 / 96.0)  // ==> %p0
  );
}
...
ENTRY main {
  ...
  EXPECT_EQ(result->flops,
            514 * 16 * 1024 * 1                          // ==> %dot
                + 86 * 16 * 2048 * CeilOfRatio(257, 64)  // ==> %exp
  );
  // The bytes read contribution for a single instruction is calculated as:
  // element_type_size * num_blocks * tile_size * multiplier
  EXPECT_EQ(result->bytes_read,
            4 * 16 * (64 * 32) * CeilOfRatio(257, 64)        // ==> %p1
                + 4 * 16 * (64 * 32) * CeilOfRatio(257, 64)  // ==> %p0
  );
}
