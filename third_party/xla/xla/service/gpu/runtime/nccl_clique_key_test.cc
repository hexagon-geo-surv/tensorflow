...
const NcclCliqueKey kBaseCliqueKey =
    NcclCliqueKey({GlobalDeviceId(0), GlobalDeviceId(1)}, NcclStreamId(0),
...
TEST(NcclCliqueKeyGettersTest, Devices) {
  EXPECT_THAT(
      kBaseCliqueKey.devices(),
      ::testing::UnorderedElementsAre(GlobalDeviceId(0), GlobalDeviceId(1)));
}

TEST(NcclCliqueKeyGettersTest, Rank) {
  EXPECT_EQ(kBaseCliqueKey.rank(GlobalDeviceId(0)), 0);
  EXPECT_EQ(kBaseCliqueKey.rank(GlobalDeviceId(1)), 1);
  EXPECT_EQ(kBaseCliqueKey.rank(GlobalDeviceId(2)), std::nullopt);
  EXPECT_EQ(kBaseCliqueKey.rank(GlobalDeviceId(3)), std::nullopt);
}

TEST(NcclCliqueKeyGettersTest, StreamId) {
  EXPECT_EQ(kBaseCliqueKey.stream_id(), NcclStreamId(0));
}

TEST(NcclCliqueKeyGetterTest, ToString) {
  EXPECT_EQ(kBaseCliqueKey.ToString(),
