...
bool CanFuseDynamicSlice(const HloDynamicSliceInstruction& dynamic_slice) {
  if (CodegenDecision decision = IsTritonSupportedDynamicSlice(dynamic_slice);
      !decision.CanFuse()) {
    VLOG(5) << "Not fusing " << dynamic_slice.ToString()
            << " to the output due to the decision: " << decision.Explain();
    return false;
  }
  // TODO(b/417172838): this check replicates the legacy emitter behavior. New
  // emitter supports dynamic slice for all dimensions but that will be enabled
  // separately.
...
FusionPlanAndRequirements BuildFusionPlanTowardOperands(
  ...
  while (queue.size() > num_requeued) {
    ...
    if (original_hlo.opcode() == HloOpcode::kDynamicSlice) {
      ...
      if (!CanFuseDynamicSlice(dynamic_slice)) {
        fusion_builder.SetShouldFuseNode(node_id, false);
        continue;
      }
    }
