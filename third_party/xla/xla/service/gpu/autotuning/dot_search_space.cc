...
TritonDotFusionSearchSpace::OptimizeConfigsForCompilationPerformance(
  ...
  for (TritonGemmConfig config : hints) {
    ...
    VLOG(10) << "Adding config to filter: " << config.ToString();
  ...
  for (const TritonGemmConfig& config : configs) {
    ...
    VLOG(10) << "Filtering out configs for compilation performance: surviving "
                "config = "
             << config.ToString();
  ...
  if (result_configs.empty()) {
    LOG(WARNING) << "All configs were filtered out for compilation performance "
                    "optimization. The hints set might not contain a good "
                    "representative set of configs. Working around this by "
                    "using the full hints set instead.";
    return hints;
  }
