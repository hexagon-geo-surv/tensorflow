...
std::pair<int64_t, int64_t> ParametersFromOperands(const HloInstruction& op) {
  const HloParameterInstruction* param0 =
      DynCast<HloParameterInstruction>(op.operand(0));
  const HloParameterInstruction* param1 =
      DynCast<HloParameterInstruction>(op.operand(1));
...
std::optional<SortComputationAnalysis> AnalyzeCompareOp(
  ...
  auto [index0, index1] = ParametersFromOperands(*compare);
...
std::optional<SortComputationAnalysis> AnalyzeComplexSortComputation(
  ...
  auto [iotap0, iotap1] = ParametersFromOperands(*root->operand(1));
  if (iota_cmp == nullptr ||
  ...
  auto [p0, p1] = ParametersFromOperands(*eq_cmp);
  if (p0 < 0 || p1 < 0) {
    ...
    auto [a, b] = ParametersFromOperands(*cmp);
    auto [p, q] = ParametersFromOperands(*cmp_reverse);
