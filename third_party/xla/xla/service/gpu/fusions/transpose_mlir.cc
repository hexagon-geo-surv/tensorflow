...
IndexingMap CreateIdentityMap(std::vector<int64_t> ranges, MLIRContext* ctx) {
  int num_dims = ranges.size();
  SmallVector<AffineExpr> identity_results;
  identity_results.reserve(num_dims);
  std::vector<DimVar> identity_dims;
  identity_dims.reserve(num_dims);
  for (auto [i, range] : llvm::enumerate(ranges)) {
    identity_results.push_back(mlir::getAffineDimExpr(i, ctx));
    identity_dims.push_back(DimVar{Interval{0, range}});
  }
  auto identity_map = mlir::AffineMap::get(num_dims, 0, identity_results, ctx);
  return IndexingMap(identity_map, identity_dims,
                     /*range_vars=*/{}, /*rt_vars=*/{});
}
