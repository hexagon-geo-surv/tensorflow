...
// CHECK-SAME: (%[[MAP_RESULT1:.*]], %[[MAP_RESULT2:.*]]) in #[[$MAP1]]
// CHECK-SAME: iter_args(%[[TENSOR:.*]] = %[[OUTPUT]])

// CHECK: %[[SCALAR:.*]] = vector.extract %{{.*}}[%[[S0]], %[[S1]]] : f32 from vector<2x2xf32>
// CHECK: %[[NEW_TENSOR:.*]] = tensor.insert %[[SCALAR]] into %[[TENSOR]][%[[MAP_RESULT1]], %[[MAP_RESULT2]]]
...
#map = #xla_gpu.indexing_map<(d0, d1)[s0, s1] -> (d1*32+d0*2+s0, s1), domain: d0 in [0, 32], d1 in [0, 8], s0 in [0, 1], s1 in [0, 1]>
#map1 = #xla_gpu.indexing_map<(d0, d1)[s0, s1] -> (d0*2+s0, s1), domain: d0 in [0, 32], d1 in [0, 2], s0 in [0, 1], s1 in [0, 1]>
#map2 = #xla_gpu.indexing_map<(d0, d1)[s0, s1] -> (s0, s1), domain: d0 in [0, 32], d1 in [0, 2], s0 in [0, 1], s1 in [0, 1]>
