...
ScalarOrTensor Broadcast(ImplicitLocOpBuilder& b, TensorValue value,
  ...
  return Wrap(b.create<mt::BroadcastOp>(value.getType().clone(shape), value));
}

ScalarOrTensor Range(ImplicitLocOpBuilder& b, int32_t limit) {
  auto type = mlir::RankedTensorType::get(limit, b.getI32Type());
  return Wrap(b.create<mt::MakeRangeOp>(type, 0, limit));
}
...
ScalarOrTensor EmitParameterLoad(ImplicitLocOpBuilder& b, Value pointer,
  ...
  if (auto make_tensor_ptr = pointer.getDefiningOp<mt::MakeTensorPtrOp>()) {
    if (make_tensor_ptr.getOffsets().empty()) {
      return Wrap(b.create<mt::LoadOp>(make_tensor_ptr.getBase(),
  ...
  if (mt::isTensorPointerType(pointer.getType())) {
    ...
    return Wrap(b.create<mt::LoadOp>(pointer, boundary_checks, padding,
                                     mt::CacheModifier::NONE,
                                     mt::EvictionPolicy::NORMAL,
                                     /*isVolatile=*/false));
  }
  ...
  // Non-tensor pointer.
  return Wrap(b.create<mt::LoadOp>(pointer, mt::CacheModifier::NONE,
                                   mt::EvictionPolicy::NORMAL,
                                   /*isVolatile=*/false));
}
...
absl::StatusOr<ScalarOrTensor> EmitReduce(
  ...
  if (input_reduction_dimension_size !=
    ...
    Value range = Range(b, input_reduction_dimension_size).Unwrap();
    ...
    Value mask =
        Broadcast(b, mlir::cast<TensorValue>(range), input_shape).Unwrap();
    ScalarOrTensor constant = CreateConst(
        b, b.getI32Type(), source_tensor_reduction_dimension_size, input_shape);
    mask =
        b.create<ma::CmpIOp>(ma::CmpIPredicate::slt, mask, constant.Unwrap());

    ScalarOrTensor neutral = values[tiled_hlo_reduce.operand(1)];
    // Triton's broadcast requires that the rank of the source and broadcasted
    // result are equal.
    if (auto* scalar = std::get_if<ScalarValue>(&neutral.value)) {
      neutral = Splat(b, neutral, input_shape);
    } else {
      for (int i = 0; i < input_shape.size(); i++) {
        neutral =
            Wrap(b.create<mt::ExpandDimsOp>(neutral.Unwrap(), /*axis=*/0));
      }
      neutral =
          Broadcast(b, mlir::cast<TensorValue>(neutral.Unwrap()), input_shape);
    }
    input =
        Wrap(b.create<ma::SelectOp>(mask, input.Unwrap(), neutral.Unwrap()));
  }

  mt::ReduceOp reduction =
      b.create<mt::ReduceOp>(input.Unwrap(), reduction_dimension);
  {
    ...
    for (const HloInstruction* instr :
      ...
      if (instr->opcode() == HloOpcode::kParameter) {
        ...
        TF_RET_CHECK(
            region_values
                .insert({instr, Wrap(reducer->getArgument(parameter_number))})
                .second);
      } else {
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        EmitScope(b, libdevice_path, device_info, /*analysis=*/nullptr, to_emit,
                  region_values));
    b.create<mt::ReduceReturnOp>(SmallVector<Value>({result.Unwrap()}));
    b.setInsertionPointAfter(reduction);
  }

  return Wrap(reduction.getResult().front());
}
...
ScalarOrTensor EmitTiledBroadcast(
  ...
  if (std::holds_alternative<ScalarValue>(input.value)) {
    return Splat(b, input, padded_output_tile_shape);
  }

  Value expanded_input = input.Unwrap();
...
absl::StatusOr<ScalarOrTensor> EmitTiledIota(
  ...
  Value range = b.create<ma::MulIOp>(
      Range(b, padded_tile_sizes[iota_dim]).Unwrap(),
      Splat(b,
  ...
  range = b.create<ma::AddIOp>(
      range,
      Splat(b, Wrap(iota_dim_offset), padded_tile_sizes[iota_dim]).Unwrap());
...
ScalarOrTensor EmitTiledReshape(ImplicitLocOpBuilder& b,
  ...
  // At this point we know that the input is a non-OD tensor.
  ...
  auto reshape = b.create<mt::ReshapeOp>(output_tensor_type, input.Unwrap(),
                                         allow_reorder);
  return Wrap(reshape.getResult());
}
...
Value EmitTiledBitcast(ImplicitLocOpBuilder& b,
  ...
  Value normalized_reshape =
      ShapeUtil::Equal(trt.transpose1_shape, trt.reshape_shape)
          ? normalized_input
          : EmitTiledReshape(b, reshape_tile_sizes, Wrap(normalized_input))
                .Unwrap();
...
absl::StatusOr<ScalarOrTensor> EmitTiledHloInstruction(
  ...
  if (fusion->IsUserOf(hlo)) {
    ...
    if (expected_element_type != loaded_element_type) {
      ...
      parameter = Wrap(Cast(b, parameter.Unwrap(), expected_element_type));
    }
  ...
  if (hlo->IsElementwise()) {
    ...
    for (const TiledHloInstruction* operand : tiled_hlo.operands()) {
      operands.push_back(values[operand].Unwrap());
    }
    TF_ASSIGN_OR_RETURN(
        Value result,
  ...
  if (hlo->opcode() == HloOpcode::kBitcast) {
    return Wrap(
        EmitTiledBitcast(b, tiled_hlo, values[tiled_hlo.operand(0)].Unwrap()));
  }

  if (hlo->opcode() == HloOpcode::kTranspose) {
    auto transpose =
        ::xla::Cast<const HloTransposeInstruction>(tiled_hlo.hlo());
    return Wrap(EmitTiledTranspose(b, tiled_hlo.tile_sizes(),
                                   llvm::to_vector(transpose->dimensions()),
                                   values[tiled_hlo.operand(0)].Unwrap()));
  }
...
absl::StatusOr<MakeTensorPtrOpAndBoundaryChecks> CreateMakeTensorPtrOp(
  ...
  for (int dim_idx = 0; dim_idx < padded_tile_sizes.size(); ++dim_idx) {
    ...
    //     the offsets.
    Value parent_size =
        CreateConst(b, b.getI64Type(), shape.dimensions(dim_idx)).Unwrap();
    ...
    offsets.push_back(CreateConst(b, b.getI32Type(), 0).Unwrap());
...
absl::Status EmitGeneric(mlir::OpBuilder builder,
  ...
  if (result_element_type != result_storage_type) {
    result = Wrap(Cast(b, result.Unwrap(), result_storage_type));
  }
  ...
  if (auto* scalar = std::get_if<ScalarValue>(&result.value)) {
    b.create<mt::StoreOp>(parent_base_ptr, scalar->value,
  ...
  b.create<mt::StoreOp>(make_tensor.op, result.Unwrap(),
