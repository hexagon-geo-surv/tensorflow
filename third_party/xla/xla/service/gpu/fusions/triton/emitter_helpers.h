...
struct ScalarOrTensor {
  mlir::Value Unwrap() {
    if (std::holds_alternative<ScalarValue>(value)) {
      return std::get<ScalarValue>(value).value;
    }
    return std::get<TensorValue>(value).value;
  }

  mlir::Type getType() { return Unwrap().getType(); }

  std::variant<ScalarValue, TensorValue> value;
};

// Wraps the given value in a ScalarOrTensor. By default, CHECK-fails if the
// value is a 0D tensor, because Triton does not support 0D tensors.
//
// To enable various hacks in the legacy matmul emitter to function, the
// check-fail can be disabled by setting `allow_rank_0` to true.
//
// Only code inside `triton_fusion_emitter_legacy_matmul` should set
// allow_rank_0 should be set to `true` here and in the rest of the functions
// in this file.
ScalarOrTensor Wrap(mlir::Value value, bool allow_rank_0 = false);
...
ScalarOrTensor CreateConst(mlir::ImplicitLocOpBuilder b, mlir::Type type,
  ...
  if (mlir::isa<mlir::IntegerType>(type)) {
    ...
    return Wrap(result, /*allow_rank_0=*/false);
  }
...
ScalarOrTensor CreateConst(mlir::ImplicitLocOpBuilder& b, mlir::Type type,
                           ...
                           bool allow_rank_0 = false) {
  if (shape.empty() && !allow_rank_0) {
  ...
  if (auto int_type = mlir::dyn_cast<mlir::IntegerType>(type)) {
    ...
    return Wrap(result, allow_rank_0);
  }
...
mlir::Value ConstLike(mlir::ImplicitLocOpBuilder& b, mlir::Value like,
                      T new_value, bool allow_rank_0 = false) {
...
absl::StatusOr<mlir::Value> EmitElementwise(
    ...
    mlir::ValueRange inputs, bool allow_rank_0 = false);
