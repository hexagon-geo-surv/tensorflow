...
ScalarOrTensor Wrap(mlir::Value value, bool allow_rank_0) {
  if (auto tt = mlir::dyn_cast<mlir::RankedTensorType>(value.getType())) {
    if (!allow_rank_0) {
      CHECK_GT(tt.getRank(), 0);
    }
    return ScalarOrTensor{TensorValue{value}};
  } else {
    return ScalarOrTensor{ScalarValue{value}};
  }
}
  ...
  if (dst_element_ty.isBF16()) {
    ...
    if (!src_element_ty.isInteger(8)) {
      return b.create<ma::TruncFOp>(
          dst_ty, Cast(b, value, b.getF32Type(), allow_rank_0));
    }
  }
  ...
  if (src_fp_element_ty && mlir::isa<mlir::IntegerType>(dst_element_ty)) {
    if (dst_element_ty.isInteger(1)) {
      return b.create<ma::CmpFOp>(ma::CmpFPredicate::UNE, value,
                                  ZerosLike(b, value, allow_rank_0));
    }
    // TODO(b/266862493): Support unsigned integer types.
    ...
    auto cst_int = [&](int64_t x) {
      if (auto src_shaped_ty = mlir::dyn_cast<ShapedType>(src_ty)) {
        ...
        return CreateConst(b, dst_element_ty, x).Unwrap();
      }
    };
    auto cst_float = [&](int64_t x) {
      if (auto src_shaped_ty = mlir::dyn_cast<ShapedType>(src_ty)) {
        ...
        return CreateConst(b, src_fp_element_ty, x).Unwrap();
      }
    };
...
ScalarOrTensor Splat(ImplicitLocOpBuilder& b, ScalarOrTensor value,
                     ArrayRef<int64_t> shape, bool allow_rank_0) {
  CHECK(allow_rank_0 || !shape.empty());
  auto type = mlir::RankedTensorType::get(shape, value.getType());
  return Wrap(b.create<mt::SplatOp>(type, value.Unwrap()), allow_rank_0);
}
...
absl::StatusOr<Value> EmitElementwise(ImplicitLocOpBuilder& b,
                                      ...
                                      ValueRange inputs, bool allow_rank_0) {
  ...
  switch (hlo.opcode()) {
    ...
    case HloOpcode::kNot:
      return b.create<ma::XOrIOp>(inputs[0],
                                  OnesLike(b, inputs[0], allow_rank_0));
    case HloOpcode::kNegate:
      // NegFOp is not supported by Triton.
      return Subtract(b, {ZerosLike(b, inputs[0], allow_rank_0), inputs[0]});
    case HloOpcode::kConvert: {
      TF_ASSIGN_OR_RETURN(Type dst_ty,
                          TritonType(b, hlo.shape().element_type()));
      return Cast(b, inputs[0], dst_ty, allow_rank_0);
    }
    case HloOpcode::kAdd:
      if (is_integer) {
    ...
    case HloOpcode::kSelect:
      return b.create<ma::SelectOp>(
          Compare(b, {inputs[0], ZerosLike(b, inputs[0], allow_rank_0)},
...
absl::StatusOr<Value> EmitUnpackInt4(ImplicitLocOpBuilder& b,
  ...
  Value shift4 = Splat(b, CreateConst(b, b.getI8Type(), 4),
                       input_type.getShape(), allow_rank_0)
                     .Unwrap();
