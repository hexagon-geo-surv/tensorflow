...
Value SplatWith0D(ImplicitLocOpBuilder& b, Value value,
                  ArrayRef<int64_t> shape) {
  return triton::Splat(b, triton::Wrap(value, /*allow_rank_0=*/true), shape,
                       /*allow_rank_0=*/true)
      .Unwrap();
}
...
absl::StatusOr<Value> EmitBroadcast(ImplicitLocOpBuilder& b,
  ...
  if (!tensor_input) {
    // Input is scalar.
    return SplatWith0D(b, input, out_shape);
  }
  ...
  for (const HloInstruction* hlo : instructions) {
    ...
    if (hlo->opcode() == HloOpcode::kConvert &&
      ...
      TF_ASSIGN_OR_RETURN(result,
                          EmitElementwise(b, libdevice_path, device_info, *hlo,
                                          operands, /*allow_rank_0=*/true));
      ...
      TF_ASSIGN_OR_RETURN(ScalarOrTensor constant, EmitConstant(b, *hlo));
      // Splat makes it a tensor to avoid type mismatches.
      result = SplatWith0D(b, constant.Unwrap(), {});
...
class MatMulEmitterHelper {
 ...
 private:
  ...
  Value Cst(int64_t v) { return CreateConst(b_, index_ty_, v).Unwrap(); }
  Value Cst32(int32_t v) { return CreateConst(b_, i32_ty_, v).Unwrap(); }
  Value Cst64(int64_t v) { return CreateConst(b_, i64_ty_, v).Unwrap(); }
...
Value TruncateToBF16TowardsZero(ImplicitLocOpBuilder& b, Value input) {
  ...
  Value mask =
      CreateConst<uint32_t>(b, b.getI32Type(), 0xFFFF0000u,
                            input_type.getShape(), /*allow_rank_0=*/true)
          .Unwrap();
...
absl::StatusOr<Value> Emit6xBfloat16MatMul(ImplicitLocOpBuilder& b, Value lhs,
  ...
  Value local_acc = ZerosLike(b, acc, /*allow_rank_0=*/true);
  ...
  result = b.create<ma::SelectOp>(is_finite, result,
                                  ZerosLike(b, result, /*allow_rank_0=*/true));
...
class Scopes {
 public:
  Scopes(ImplicitLocOpBuilder& b, const HloInstruction* dot_instr,
    ...
    auto c32 = [&](int64_t v) {
      return CreateConst(b, b.getI32Type(), v).Unwrap();
    };
      ...
      [&](mlir::OpBuilder& builder, mlir::Location loc) {
        ...
        if (pid_k != nullptr) {
          range_k = b.create<ma::AddIOp>(
              range_k,
              SplatWith0D(b, b.create<ma::MulIOp>(pid_k, c32(size)), size));
        }
        ...
        Value mask = b.create<mt::BroadcastOp>(
            ...
            b.create<ma::CmpIOp>(
                ...
                SplatWith0D(b, elements_in_tile,
                            range_expanded.getType().getShape())));
        auto result = b.create<ma::SelectOp>(
            mask, input, ZerosLike(b, input, /*allow_rank_0=*/true));
...
absl::Status EmitMatMul(mlir::OpBuilder builder,
  ...
  Value accumulator_init =
      CreateConst(b, acc_ty, 0, {block_m, block_n}, /*allow_rank_0=*/true)
          .Unwrap();
