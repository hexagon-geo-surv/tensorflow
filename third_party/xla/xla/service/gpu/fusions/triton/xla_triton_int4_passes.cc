...
// Add an integer constant to a value.
template <typename Rewriter>
Value add(Rewriter &r, Value a, int64_t c) {
  return r.template create<arith::AddIOp>(
      a.getLoc(), a,
      r.template create<ma::ConstantOp>(a.getLoc(),
                                        r.getIntegerAttr(a.getType(), 1)));
}

// Divide a value by an integer constant.
template <typename Rewriter>
Value div(Rewriter &r, Value a, int64_t c) {
  return r.template create<arith::DivSIOp>(
      a.getLoc(), a,
      r.template create<ma::ConstantOp>(a.getLoc(),
                                        r.getIntegerAttr(a.getType(), c)));
}
...
class AdvanceOpConversionPattern : public OpConversionPattern<AdvanceOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
      ...
      ConversionPatternRewriter &r) const override {
    VLOG(10) << "AvanceOpConversionPattern: matching\n"
             << DumpToString(static_cast<Operation *>(op.getOperation()));
    // Convert the tensor type using the TypeConverter
    auto new_type = converter_.convertType(op.getType());
    if (op.getType() == new_type) {
      VLOG(10) << "AdvanceOpConversionPattern: no conversion needed for "
...
// The pattern converts the ExtSIOp that converts i4 tensor to i8 tensor to
// the unpack sequence with ShLIOp, ShRSIOp, JoinOp, TransOp and ReshapeOp
...
class ExtSIInt4ToInt8Pattern : public OpConversionPattern<ma::ExtSIOp> {
 public:
  ...
  LogicalResult matchAndRewrite(ma::ExtSIOp op, OpAdaptor adaptor,
                                ConversionPatternRewriter &r) const override {
    VLOG(10) << "ExtSIInt4ToInt8Pattern: matching\n"
...
// Traverse the operands of the op passing though the forOp.
...
// Find all the ExtSIOp that require the type conversion.
std::vector<Operation *> FindInt4ExtSIOp(const ModuleOp &module) {
  ...
  module->walk([&](Operation *op) {
    if (auto extSI = dyn_cast<arith::ExtSIOp>(op)) {
      VLOG(10) << "found ExtSI: " << DumpToString(op);
...
// Check which dimension is packed. We use packed_dim attribute to determine
// which dimension is packed. If the tensor is single dimension, then we invert
// the result. The tensor (Nx1) which is packed along the minor dimension, but
// every byte has two i4 elements belonging to different rows, so the tensor is
// packed along the major dimension and vice versa.
I4ToI8Converter::PackedDim GetPackedDim(MLIRContext *ctx,
  ...
  for (auto *op : ops) {
    ...
    if (attr_dict.contains("packed_dim")) {
      ...
      VLOG(10) << "packed_dim: " << packed_dim_attr.getValue().str();
...
struct PlainInt4ToPackedInt4RewritePass
  ...
  // the further processing. It gets the packed dimension from the
  // MakeTensorPtrOp attribute.
  void runOnOperation() override {
    ...
    // differently.
    for (auto *op : ext_ops) {
      VLOG(10) << "ext_op: " << DumpToString(op);
