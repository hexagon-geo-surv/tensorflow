...
class RewriteAtomicRMW : public mlir::OpRewritePattern<AtomicRMWOp> {
 ...
 private:
  ...
  std::optional<std::pair<Value, ml::AtomicBinOp>> GetAtomicModifierParameters(
    ...
    std::optional<ml::AtomicBinOp> kind =
        llvm::TypeSwitch<Operation*, std::optional<ml::AtomicBinOp>>(
            ...
            // Integer operations.
            .Case([](arith::AddIOp op) { return ml::AtomicBinOp::add; })
            .Case([](arith::MaxUIOp op) { return ml::AtomicBinOp::umax; })
            .Case([](arith::MinUIOp op) { return ml::AtomicBinOp::umin; })
            .Case([](arith::MaxSIOp op) { return ml::AtomicBinOp::max; })
            .Case([](arith::MinSIOp op) { return ml::AtomicBinOp::min; })
  ...
  LogicalResult rewriteAsDirectAtomicRMW(
    ...
    switch (atomic_bin_op) {
      case ml::AtomicBinOp::xchg: {
        return emitAtomicStoreOp(loc, modifier_arg, addr, rewriter);
      }
      ...
      case ml::AtomicBinOp::umin: {
        return emitIntAtomicRMWOp(loc, atomic_bin_op, modifier_arg, addr,
                                  sync_scope, rewriter);
      }
  ...
  LogicalResult emitIntAtomicRMWOp(Location loc, ml::AtomicBinOp atomic_bin_op,
                                   Value modifier_arg, Value addr,
                                   llvm::StringRef sync_scope,
                                   OpBuilder& b) const {
    mlir::Type element_type = modifier_arg.getType();
    if (!IsAtomicIntegral(element_type)) {
      return failure();
    }
    b.create<ml::AtomicRMWOp>(loc, atomic_bin_op, addr, modifier_arg,
                              ml::AtomicOrdering::seq_cst, sync_scope);
    return success();
  }

  LogicalResult emitAtomicStoreOp(Location loc, Value modifier_arg, Value addr,
                                  OpBuilder& b) const {
    mlir::Type element_type = modifier_arg.getType();
    if (!element_type.isF32() && !IsAtomicIntegral(element_type)) {
      return failure();
    }
    b.create<ml::StoreOp>(loc, modifier_arg, addr,
  ...
  LogicalResult rewriteAtomicFMaxAsIntAtomics(Location loc, Value modifier_arg,
    ...
    auto is_current_nan = b.create<scf::IfOp>(
        loc, /*resultTypes=*/mlir::TypeRange{}, current_is_nan,
    ...
    auto F32NaN = mlir::APFloat::getNaN(mlir::APFloat::IEEEsingle());
    Value nan = if_current_nan_else_builder.create<ml::ConstantOp>(
        loc, b.getF32Type(), F32NaN);
    ...
    auto if_need_update = if_current_nan_else_builder.create<scf::IfOp>(
        loc, /*resultTypes=*/mlir::TypeRange{}, current_less_than_modifier,
        /*withElseRegion=*/true,
        /*addElseBlock=*/false);
    ...
    then_builder.create<scf::IfOp>(
        ...
        [&](OpBuilder& nested_b, Location nested_loc) {
          ...
          (void)emitIntAtomicRMWOp(nested_loc, ml::AtomicBinOp::max,
                                   source_float_as_int, addr, sync_scope,
                                   nested_b);
          nested_b.create<scf::YieldOp>(nested_loc);
        },
        [&](OpBuilder& nested_b, Location nested_loc) {
          // atomicMax((int *)address, __float_as_int(val))
          (void)emitIntAtomicRMWOp(nested_loc, ml::AtomicBinOp::umin,
                                   source_float_as_int, addr, sync_scope,
                                   nested_b);
          nested_b.create<scf::YieldOp>(nested_loc);
        });
