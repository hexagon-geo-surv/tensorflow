...
CutlassGemmWithUpcastPattern::TryMatch(const se::DeviceDescription& device,
  ...
  // Cutlass doesn't support F32xBF16->F32 gemms.
  if (matched->lhs_upcast == nullptr &&
      matched->dot->operand(0)->shape().element_type() == PrimitiveType::F32 &&
      matched->rhs_upcast != nullptr &&
      matched->rhs_upcast->operand(0)->shape().element_type() ==
          PrimitiveType::BF16) {
    return std::nullopt;
  }
