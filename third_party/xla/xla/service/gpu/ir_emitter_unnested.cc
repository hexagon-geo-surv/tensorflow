...
absl::Status IrEmitterUnnested::EmitNcclGroupThunk(const HloInstruction* instr,
                                                   Thunk::Kind kind) {
  ThunkSequence group_thunks;
  scoped_thunk_sequence_ = &group_thunks;
  ...
  auto thunk =
      std::make_unique<NcclGroupThunk>(instr, kind, std::move(group_thunks));
  // TODO (rosiezou): use absl cleanup to automatically set this to nullptr if
  // the function preemptively errors out.
  scoped_thunk_sequence_ = nullptr;
  AddThunkToThunkSequence(std::move(thunk));
  return absl::OkStatus();
}
...
absl::Status IrEmitterUnnested::EmitSendThunk(const HloSendInstruction* instr) {
  ...
  if (!instr->is_host_transfer()) {
    ...
    // group thunk.
    if (scoped_thunk_sequence_ == nullptr) {
