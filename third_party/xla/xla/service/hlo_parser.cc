...
HloInstruction* HloParserImpl::CreateInstruction(  // NOLINT
  ...
  switch (opcode) {
    ...
    case HloOpcode::kAllGather:
    case HloOpcode::kAllGatherStart: {
      optional<CollectiveDeviceList> device_list;
      ...
      if (opcode == HloOpcode::kAllGather) {
        return builder->AddInstruction(HloInstruction::CreateAllGather(
            *shape, operands, dimensions->at(0),
            device_list.value_or(CollectiveDeviceList()),
    ...
    case HloOpcode::kAllToAll: {
      ...
      return builder->AddInstruction(HloInstruction::CreateAllToAll(
          *shape, operands, device_list.value_or(CollectiveDeviceList()),
          constrain_layout ? *constrain_layout : false, channel_id,
          split_dimension));
    }
    case HloOpcode::kCollectiveBroadcast: {
      ...
      return builder->AddInstruction(HloInstruction::CreateCollectiveBroadcast(
          *shape, operands, device_list.value_or(CollectiveDeviceList()), false,
          channel_id));
    }
    case HloOpcode::kCollectivePermute:
    case HloOpcode::kCollectivePermuteStart: {
...
// ::= '{' <legacy_list> '}' | iota_list
// legacy_list ::= '{' <int_list> '}' ( ',' '{' <int_list> '}' )*
  ...
  bool success = [&] {
    ...
    switch (attr_type) {
      ...
      case AttrTy::kCollectiveDeviceList: {
        ...
        static_cast<optional<CollectiveDeviceList>*>(attr_out_ptr)
            ->emplace(device_list);
        return true;
      }
