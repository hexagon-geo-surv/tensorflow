...
TEST_F(CallOutlinerTest, OutlineNestedBlockWithDeadInstruction) {
  const absl::string_view hlo_string = R"(
  HloModule nested_dead_inst_module
  ...
  ENTRY entry {
    ...
    dead_op = f32[2,2]{1,0} negate(gte_outer)
    before_inner = (f32[2,2]{1,0}) custom-call(gte_outer), custom_call_target="__xla_internal_call_marker_before", frontend_attributes={xla_call_marked_computation="inner"}
    gte_inner = f32[2,2]{1,0} get-tuple-element(before_inner), index=0
    add_inner = f32[2,2]{1,0} add(gte_inner, gte_inner)
    ...
    add_outer = f32[2,2]{1,0} add(after_inner, gte_outer)
  ...
  EXPECT_EQ(root_call->operand_count(), 1);
  EXPECT_EQ(root_call->to_apply()->instruction_count(), 3);
}
