...
absl::Status AOTInterceptionPjrtClient::CompareGoldenGPUExecutable(
    const HumanReadableAotExecutable& fresh,
    const HumanReadableAotExecutable& golden) {
  HumanReadableAotExecutable fresh_normalized = fresh;
  HumanReadableAotExecutable golden_normalized = golden;
  ...
  // excluded, and normalize only the latter.
  auto normalize = [](HumanReadableAotExecutable& executable) {
  ...
  normalize(fresh_normalized);
  normalize(golden_normalized);
  ...
  if (!differencer.Compare(golden_normalized, fresh_normalized)) {
...
absl::Status AOTInterceptionPjrtClient::VerifyAgainstGolden(
  ...
  return CompareGoldenGPUExecutable(fresh_unpacked, golden);
}
