 ...
 public:
  ...
  void CreateRandomTestData(HloModule* module, Array<IndexType> input_sizes) {
    ...
    auto get_row = [&](int64_t row_id, const Array<IndexType>& data) {
      Array<IndexType> row = data.Slice(
          {row_id, 0, 0}, {row_id + 1, num_replicas, num_updates_per_replica});
      row.Reshape({num_replicas, num_updates_per_replica});
      return row;
    };
...
XLA_TEST_P(RaggedAllToAllTest, RaggedAllToAll_2GPUs_MultipleUpdates) {
  ...
  ENTRY entry {
    ...
    input_offsets = s32[2,2] parameter(2)
    send_sizes = s32[2,2] parameter(3)
    output_offsets = s32[2,2] parameter(4)
    recv_sizes = s32[2,2] parameter(5)
...
XLA_TEST_P(RaggedAllToAllTest, RaggedAllToAll_8GPUs) {
  ...
  ENTRY entry {
    ...
    input_offsets = s32[8, 4] parameter(2)
    send_sizes = s32[8, 4] parameter(3)
    output_offsets = s32[8, 4] parameter(4)
    recv_sizes = s32[8, 4] parameter(5)
