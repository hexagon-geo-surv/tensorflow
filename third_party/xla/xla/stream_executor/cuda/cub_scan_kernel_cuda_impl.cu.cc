...
absl::Status CubScanImpl(void* d_temp_storage, size_t& temp_bytes,
                         const KeyT* d_in, KeyT* d_out, int64_t vector_length,
...
absl::Status CubScanDispatch(xla::PrimitiveType type, void* d_temp_storage,
  ...
  auto impl = [&](auto key) {
    ...
    return CubScanImpl<KeyT>(
        d_temp_storage, temp_bytes, static_cast<const KeyT*>(d_in),
        static_cast<KeyT*>(d_out), vector_length, row_length, column_length,
        kind, is_reverse, stream);
  };
  switch (type) {
