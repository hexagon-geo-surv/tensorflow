 ...
 protected:
  ...
  absl::Status RunCubScanTest(xla::PrimitiveType type, size_t vector_length,
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        CheckNotNull(executor_->AllocateArray<T>(num_elements), "data"));
    auto data_cleanup =
...
TEST_P(CubScanKernelCudaTest, TestPrefixSum) {
  ...
  auto impl = [&](auto value) {
    TF_EXPECT_OK(RunCubScanTest<decltype(value)>(
        type, vector_length, row_length, col_length, kind, is_reverse));
  };
  switch (type) {
    case xla::PrimitiveType::BF16:
      if (row_length > 128) {
        GTEST_SKIP() << "Skipping BF16 test for row length > 128";
      }
...
std::string ParametersToString(
  ...
  const auto& [type, vector_length, row_length, col_length, kind, is_reverse] =
      data.param;
  return absl::StrFormat("%s_%dx%dx%d_%s_%s", CubScanKindToString(kind),
                         col_length, row_length, vector_length,
