...
class HostKernel : public Kernel {
 public:
  ...
  // overheads). It's up the the caller to define where task runner will execute
  // the task, i.e. a common case is to launch them on a thread pool.
  //
  // Returned async value becomes available after all tasks are completed. Async
  // value returned in constructed state and the caller can access it to get
  // the number of tasks that are expected to be completed.
