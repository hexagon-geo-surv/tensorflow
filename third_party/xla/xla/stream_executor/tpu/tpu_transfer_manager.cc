...
bool TpuTransferManager::CanBufferBeAccessedNow(
  ...
  SE_DeviceAddressBase c_device_buffer{
      const_cast<void*>(
          device_buffer.opaque()),  // NOLINT(google-readability-const-cast): C
                                    // API requires non-const void*
      device_buffer.size(), device_buffer.payload()};
  return stream_executor::tpu::ExecutorApiFn()
...
absl::Status TpuTransferManager::WriteSingleTupleIndexTable(
  ...
  for (size_t i = 0; i < elements.size(); i++) {
    elements_bases.push_back(SE_DeviceAddressBase{
        const_cast<void*>(
            elements[i].opaque()),  // NOLINT(google-readability-const-cast): C
                                    // API requires non-const void*
