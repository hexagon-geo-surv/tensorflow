...
CommonPjRtClient::BufferFromHostLiteral(const LiteralSlice& literal,
  ...
  if (shape.IsTuple()) {
    return InvalidArgument(
        "Tuples are not supported in TfrtTpuClient::BufferFromHostLiteral");
  }
  ...
  ASSIGN_OR_RETURN(
      (auto [definition_event_promise, definition_event]),
  ...
  async_work_runner()->Schedule(
      [this, shape, literal, raw_buffer = std::move(raw_buffer),
       definition_event = std::move(definition_event_promise),
