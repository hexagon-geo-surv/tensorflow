...
absl::StatusOr<MaybeOwning<se::DeviceAddressAllocator>> CreateDeviceAllocator(
  ...
  for (const auto& device : devices) {
    ...
    if (allocator_config.preallocate) {
      ...
      // that we can control the thread on which the expansion happens.
      allocator->DeallocateRaw(
          allocator->AllocateRaw(tsl::Allocator::kAllocatorAlignment, 1));
    } else {
#ifdef PLATFORM_GOOGLE
      LOG(WARNING)
          << "TFRT GPU is runnign without preallocation; this may cause CUDA "
             "calls to happen inline on the calling thread, which is "
             "problematic if the calling thread is a fiber";
#endif
