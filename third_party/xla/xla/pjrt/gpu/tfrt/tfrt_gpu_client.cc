...
absl::StatusOr<std::unique_ptr<PjRtClient>> GetTfrtGpuClient(
  ...
  if (options.host_memory_allocator_factory != nullptr) {
    ...
    host_memory_allocator = options.host_memory_allocator_factory(
        [stream_executor](void* data, size_t size) {
    ...
    host_memory_allocator =
        std::make_shared<BasicHostMemoryAllocator>(std::move(allocator));
  }
