...
#include "third_party/triton/include/triton/Conversion/TritonGPUToLLVM/Passes.h"
#include "third_party/triton/include/triton/Conversion/TritonToTritonGPU/TritonToTritonGPUPass.h"
...
#include "third_party/triton/third_party/nvidia/include/NVGPUToLLVM/NVGPUToLLVMPass.h"
#include "third_party/triton/third_party/nvidia/include/TritonNVIDIAGPUToLLVM/Passes.h"
...
absl::Status TritonToLLVM(
  ...
  pm.addPass(mlir::createCSEPass());
  if (cc / 10 >= 8) {
    // TODO(slebedev): Add this pass to the XLA:GPU pipeline.
...
absl::StatusOr<std::string> LLVMToPTX(mlir::ModuleOp module,
  ...
  // "8.0" -> "80"
  auto cc = absl::StrReplaceAll(arch_name, {{".", ""}});
  ...
  TF_ASSIGN_OR_RETURN(
      auto machine, CreateTargetMachine(
                        llvmModule.get(), /*arch_name=*/"sm_" + cc,
                        /*enable_fp_fusion=*/false, /*features=*/"+ptx" + cc));
