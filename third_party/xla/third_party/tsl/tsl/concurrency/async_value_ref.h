...
class AsyncValueRef {
 public:
  ...
  template <typename Waiter>
  void AndThen(const AsyncValue::Executor& executor, Waiter&& waiter) const {
  ...
  template <typename R, typename F, typename U = std::invoke_result_t<F, T&>,
            std::enable_if_t<std::is_constructible_v<R, U>>* = nullptr>
  AsyncValueRef<R> Map(const AsyncValue::Executor& executor, F&& f) {
...
class AsyncValuePtr {
 ...
 public:
  ...
  void AndThen(Waiter&& waiter) const {
    AndThen([waiter = std::forward<Waiter>(waiter), self = *this]() mutable {
      if (ABSL_PREDICT_FALSE(self.IsError())) {
        return waiter(self.GetError());
      } else {
        return waiter(&self.get());
      }
    });
  }
  ...
  void AndThen(const AsyncValue::Executor& executor, Waiter&& waiter) const {
    AndThen(executor,
            [waiter = std::forward<Waiter>(waiter), self = *this]() mutable {
              if (ABSL_PREDICT_FALSE(self.IsError())) {
                return waiter(self.GetError());
              } else {
                return waiter(&self.get());
  ...
  AsyncValueRef<R> Map(F&& f) {
    ...
    AndThen([f = std::forward<F>(f), result, self = *this]() mutable {
      if (ABSL_PREDICT_FALSE(self.IsError())) {
        result.SetError(self.GetError());
      } else {
        result.emplace(f(self.get()));
      }
    });
    return result;
  }
  ...
  AsyncValueRef<R> Map(const AsyncValue::Executor& executor, F&& f) {
    auto result = MakeUnconstructedAsyncValueRef<R>();
    AndThen(executor, [f = std::forward<F>(f), result, self = *this]() mutable {
      if (ABSL_PREDICT_FALSE(self.IsError())) {
        result.SetError(self.GetError());
      } else {
        result.emplace(f(self.get()));
