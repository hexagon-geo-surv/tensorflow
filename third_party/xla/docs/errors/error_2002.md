...
**Category:** Mosaic Input/Output Misaligned Block and Tiling
...
dimensions are called **lane** and **sublane**. Since TPU compute units (e.g.,
...
loading a block of size `(8, 100)` from a dimension of size `(8, 1024)` on
hardware with `(8, 128)` tiling fails because `100` is not divisible by `128`
and `100 != 1024`.

## Debugging

To resolve this error, ensure your kernel's block shapes align with the
current hardware tiling. You have the following options:

### 1. Update Kernel Tiling {#update-tiling}

The most direct fix is to modify your kernel code to align the block size such
...
### 2. Use Dynamic Tiling {#dynamic-tiling}

Instead of hardcoding block sizes, you can use
[jax.experimental.pallas.tpu.TpuInfo](https://docs.jax.dev/en/latest/_autosummary/jax.experimental.pallas.tpu.TpuInfo.html)
to retrieve the correct sublane tiling for the current hardware and data type.
This ensures your kernel remains portable across different TPU generations and
flag configurations without manual code changes.