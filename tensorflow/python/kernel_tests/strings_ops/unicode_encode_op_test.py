...
class UnicodeEncodeOpTest(test.TestCase, parameterized.TestCase):
  ...
  def testIgnoreErrors(self, encoding):
    test_value = np.array([ord('H'), ord('e'), 0x7FFFFFFF, -1, ord('o'),
                           ...
                           0xD83D, 0xDE00,  # invalid, surrogate pair
                           0xFDD0,  # non-character
                           0xFFFE, 0xFFFF,  # last two in BMP
                           0x10FFFF,  # last in plane 16
                           0x110000],  # invalid, too large
                          np.int32)
    ...
    # 2. non-characters are not rejected
    # 3. last two codepoints in plane are not rejected
    expected_value = (
