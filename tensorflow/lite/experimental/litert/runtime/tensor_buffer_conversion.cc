...
#if LITERT_HAS_OPENGL_SUPPORT && LITERT_HAS_AHWB_SUPPORT
Expected<void> CopyGlToAhwb(const GlBuffer& src, AhwbBuffer& dest) {
  ...
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, src.id());
  const void* host_src =
      glMapBufferRange(GL_SHADER_STORAGE_BUFFER, src.offset(), src.bytes_size(),
                       GL_MAP_READ_BIT);
