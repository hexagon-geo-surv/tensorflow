...
#ifdef LITERT_HAS_OPENGL_SUPPORT
...
// A stub GlBuffer implementation to enable compilation.
class GlBuffer {
 public:
  static bool IsSupported() { return false; }
  static Expected<GlBuffer> Alloc(size_t bytes_size) {
    return Unexpected(kLiteRtStatusErrorRuntimeFailure,
                      "GlBuffer is not supported.");
  }

  // Allocates a CPU memory and conducts a copy from the GL buffer to the
  // CPU memory.
  template <typename T>
  Expected<T*> Lock() {
    return Unexpected(kLiteRtStatusErrorRuntimeFailure,
  ...
  // Writes the data from the CPU memory to the GL buffer.
  template <typename T>
  Expected<void> Unlock() {
    return Unexpected(kLiteRtStatusErrorRuntimeFailure,
                      "GlBuffer is not supported.");
