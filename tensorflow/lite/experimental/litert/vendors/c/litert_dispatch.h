...
// Start collection of HW-specific metrics.
LiteRtStatus LiteRtDispatchStartMetricsCollection(
    LiteRtDispatchInvocationContext invocation_context);

// Stop collection of HW-specific metrics and report the collected metrics. The
// caller is responsible for deallocating the returned metrics by calling
// `LiteRtDispatchDestroyMetrics`.
