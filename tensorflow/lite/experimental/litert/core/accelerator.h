...
using LiteRtAccelerator = LiteRtAcceleratorT*;

namespace litert::internal {

class LiteRtAcceleratorRegistry {
 public:
  struct Deleter {
  ...
  static Ptr Clone(const LiteRtAcceleratorT& accelerator) {
    return Ptr(new LiteRtAcceleratorT(accelerator));
  }
  ...
  Expected<LiteRtAcceleratorT*> RegisterAccelerator(
      LiteRtAcceleratorT* accelerator) {
    if (!accelerator) {
      return Error(kLiteRtStatusErrorInvalidArgument,
                   "Cannot register a null accelerator.");
    }
    accelerators_.push_back(LiteRtAcceleratorRegistry::Clone(*accelerator));
    return accelerators_.back().get();
  }

  LiteRtStatus Get(LiteRtParamIndex idx, LiteRtAccelerator* accelerator) {
    if (!accelerator) {
    ...
    *accelerator = accelerators_[idx].get();
 ...
 private:
  std::vector<LiteRtAcceleratorRegistry::Ptr> accelerators_;
};
