...
class LiteRtAcceleratorTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    LiteRtEnvironmentCreate(0, nullptr, &env_);
...
TEST_F(LiteRtAcceleratorTest, GetAcceleratorIdWorks) {
  ...
  ASSERT_THAT(LiteRtGetAcceleratorId(env_, accelerator, &accelerator_id),
              kLiteRtStatusOk);
  EXPECT_THAT(accelerator_id, Eq(0));

  EXPECT_THAT(LiteRtGetAcceleratorId(env_, nullptr, &accelerator_id),
              kLiteRtStatusErrorInvalidArgument);
  EXPECT_THAT(LiteRtGetAcceleratorId(env_, accelerator, nullptr),
              kLiteRtStatusErrorInvalidArgument);
  // Make the accelerator invalid.
  accelerator = reinterpret_cast<LiteRtAccelerator>(0xDEADBEEF);
  EXPECT_THAT(LiteRtGetAcceleratorId(env_, accelerator, &accelerator_id),
              kLiteRtStatusErrorNotFound);
}
