...
void FillBuffer(absl::Span<char> buffer, absl::Span<const char> data) {
  ...
  TFLITE_DCHECK(!data.empty());
  TFLITE_DCHECK_GE(buffer.size(), data.size());
  if (data.empty() || buffer.size() < data.size()) {
    return;
  }
  TFLITE_DCHECK_EQ(buffer.size() % data.size(), 0);
 ...
 public:
  ...
  TfLiteStatus Setup(TfLiteContext* context, absl::Span<const int> dims,
    ...
    for (int i = 0; i < rank; ++i) {
      ...
      const int64_t interior_gap_count =
          dims[i] > 0 ? static_cast<int64_t>(dims[i]) - 1 : 0;
      const CheckedInt<int64_t> output_dim =
          CheckedInt<int64_t>(dims[i]) +
          CheckedInt<int64_t>(interior_gap_count) * interior_pad_[i] +
    ...
    TF_LITE_ENSURE_OK(
        context, CheckedShapeProduct(context, input_dims, rank,
    ...
    TF_LITE_ENSURE_OK(
        context, CheckedShapeProduct(context, output_dims, rank,
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TfLiteIntArray* output_dims = pad_data.BuildOutputTensorDims();
