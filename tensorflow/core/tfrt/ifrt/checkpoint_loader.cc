...
absl::Status CheckpointLoader::Load(
  ...
  LOG(INFO) << "CheckpointLoader::Load: Restoring " << var_handles.size()
            << " variables across " << shards.size()
            << " shards (materialize_variables_in_resource_manager="
...
absl::Status CheckpointLoader::AwaitMutableVariables() {
  ...
  LOG(INFO) << "CheckpointLoader::AwaitMutableVariables: Waiting for "
            << futures.size() << " mutable variable ready futures...";
  const absl::Time start_time = absl::Now();
  absl::Status status = tsl::JoinFutures(absl::MakeSpan(futures)).Await();
  LOG(INFO) << "CheckpointLoader::AwaitMutableVariables: Finished waiting for "
            << futures.size() << " futures in " << (absl::Now() - start_time)
            << " with status: " << status;
  return status;
}
...
absl::Status CheckpointLoader::FreezeCleanup(
  ...
  const absl::Time start_time = absl::Now();
  LOG(INFO) << "CheckpointLoader::FreezeCleanup: Starting cleanup. Total "
               "materialized variables: "
            << materialized_variables_.size()
            << ", device variables: " << device_variables.size()
            << ", host-needed variables: " << host_needed.size();
  ...
  for (const MaterializedVariable& materialized : materialized_variables_) {
    ...
    if (!is_device_var || is_host_needed) {
      ...
      VLOG(1) << "CheckpointLoader::FreezeCleanup: Kept in ResourceManager "
                 "(is_device="
              << is_device_var << ", is_host_needed=" << is_host_needed
    ...
    if (!status.ok() && !absl::IsNotFound(status)) {
      LOG(ERROR) << "CheckpointLoader::FreezeCleanup: Failed to delete Var '"
                 << materialized.container << "/" << materialized.name
                 << "': " << status;
      return status;
    }
    deleted_count++;
    VLOG(1) << "CheckpointLoader::FreezeCleanup: Deleted device variable "
               "from ResourceManager: "
            << materialized.container << "/" << materialized.name;
  }
  materialized_variables_ = std::move(kept);
  LOG(INFO) << "CheckpointLoader::FreezeCleanup: Finished cleanup in "
            << (absl::Now() - start_time) << ". Deleted " << deleted_count
            << " variables from ResourceManager, kept "
            << materialized_variables_.size() << " host variables.";
  tsl::port::MallocExtension_ReleaseToSystem(
      std::numeric_limits<size_t>::max());
  LOG(INFO) << "CheckpointLoader::FreezeCleanup: Released free memory pages to "
               "OS via MallocExtension_ReleaseToSystem.";
  return absl::OkStatus();
}
