...
struct PriorityQueueState {
  // The number of distinct criticality bands.
  ...
  // Cumulative number of evicted (preempted) tasks per criticality.
  std::array<int64_t, kNumCriticalities> evicted_num_tasks = {};
  // Cumulative summed size of evicted (preempted) tasks per criticality.
  std::array<int64_t, kNumCriticalities> evicted_size = {};
