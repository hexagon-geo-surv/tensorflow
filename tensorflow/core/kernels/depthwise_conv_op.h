    ...
    if (kDepth > 1 && kDepth <= kPacketSize) {
      for (int64_t f_r = 0; f_r < args.filter_rows; ++f_r) {
        ...
        const auto* in = input + in_r * args.in_cols * args.in_depth +
                         in_c_start * args.in_depth - args.in_depth;
        const auto step = args.in_depth;
      ...
      const auto col_step = args.in_cols * args.in_depth;
      const auto step = args.in_depth;
      const auto start = in_c_start * step - step;
      const auto* in_col = input + in_r_start * col_step - col_step;
      for (int64_t f_r = 0; f_r < args.filter_rows; ++f_r) {
        in_col += col_step;
        const int64_t in_r = in_r_start + f_r;
