...
void DeriveEventsFromAnnotations(const SymbolResolver& symbol_resolver,
  ...
  if (tsl::profiler::GetDeviceType(*device_trace) !=
    ...
    // DeriveEventsFromAnnotations(), and find solid way to sort all lines.
    ...
    if (line_id_with_most_events >= 0) {
      DeriveEventsFromAnnotations(
          symbol_resolver, device_trace,
          absl::Span<const int64_t>(&line_id_with_most_events, 1),
          kThreadIdTfNameScope);
    }
  }
