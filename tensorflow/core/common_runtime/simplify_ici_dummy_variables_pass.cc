...
std::vector<Node*> GetNonMainReplicaIciTPUExecuteNodes(Graph* graph,
  ...
  for (Node* node : graph->nodes()) {
    if (node->type_string() == kTpuExecute &&
      ...
      // For SPMD case, we return empty vector.
      if (group_vec.size() > 1) {
...
absl::Status RedirectEdge(Graph* graph, Node* old_src_node, Node* dst_node,
  ...
  if (delete_edge == nullptr) return absl::OkStatus();
...
string GetHostDeviceName(Node* tpu_node) {
  ...
  string task_delimiter = "task:" + std::to_string(parsed_device_name.task);
  string device_name_prefix =
      device_name.substr(0, device_name.find(task_delimiter));
  string host_device_name =
      absl::StrCat(device_name_prefix, task_delimiter, "/device:CPU:0");
...
// Build the fill op. It's value is 0 and the fill op is put on the host device
...
Status SimplifyIciDummyVariablesPass::Run(
  ...
  LOG(INFO) << "DO NOT SUBMIT SimplifyIciDummyVariablesPass::Run";
