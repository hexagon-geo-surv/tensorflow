...
absl::StatusOr<std::string> XlaComputationToHloText(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      xla::HloModule::CreateFromProto(xla_computation.proto(), module_config));
...
class XlaCallModuleOp : public XlaOpKernel {
 public:
  ...
  void Compile(XlaOpKernelContext *ctx) override {
    ...
    if (VLOG_IS_ON(3)) {
      OP_REQUIRES_VALUE(std::string hlo_text, ctx,
                        XlaComputationToHloText(xla_computation.value()));
      XLA_VLOG_LINES(
          3, absl::StrCat("XlaCallModule converted to HLO module ", hlo_text));
    }
 ...
 private:
  ...
  absl::Status LowerTfFunctionCalls(XlaOpKernelContext *ctx) {
    ...
    auto lower =
      ...
      std::cout << XlaComputationToHloText(*result.computation);
      ...
      llvm::SmallVector<mlir::Value> input_args;
      {
        ...
        if (custom_call_has_token_input_output) {
          ...
          args.push_back(builder.create<mlir::stablehlo::AfterAllOp>(
              loc, mlir::stablehlo::TokenType::get(&context_),
              mlir::ValueRange{}));
        }
