...
TEST_F(DeviceCompilerTestWithDump, CompileStrictDebugInformationDumpWorks) {
  ...
  ASSERT_THAT(xla_device_compiler, ::testing::NotNull());
  core::ScopedUnref xla_device_compiler_ref(xla_device_compiler);
  ...
  EXPECT_THAT(Env::Default()->GetChildren(dump_dir_, &dump_dir_files),
              ::testing::status::IsOk());
  EXPECT_THAT(dump_dir_files, ::testing::SizeIs(1));
  ...
  EXPECT_THAT(dump_subdir_files, ::testing::UnorderedElementsAre(
                                     "options.pb", ::testing::EndsWith(".pb")));
  ...
  EXPECT_THAT(options_proto.ParseFromString(options_contents),
              ::testing::IsTrue());
  EXPECT_THAT(absl::StrCat(tsl::DeterministicProtoHash64(options_proto)),
              ::testing::StrEq(dump_subdir));
  EXPECT_THAT(options_proto.device_type(),
              ::testing::StrEq(options.device_type.type_string()));
  EXPECT_THAT(options_proto.flib_def(),
              ::testing::EqualsProto(options.flib_def->ToProto()));
  EXPECT_THAT(options_proto.graph_def_version(),
              ::testing::Eq(options.graph_def_version));
  ...
  EXPECT_THAT(compile_call_args_proto.compile_options(),
              ::testing::StrEq(compile_options.DebugString()));
  EXPECT_THAT(compile_call_args_proto.function(), ::testing::EqualsProto(fn));
  ...
  EXPECT_THAT(compile_call_args_proto.xla_arguments(),
              ::testing::ElementsAreArray(xla_arguments_human_strings));
}
