...
using ::mlir::quant::ConstantFoldOpIfPossible;
using ::mlir::quant::Create1DConstValue;
using ::mlir::quant::CreateScalarConstValue;
using ::mlir::quant::GetSplatValue;
using ::mlir::quant::HasStaticShape;
using ::mlir::quant::HasStaticShapeAtDims;
...
void PrepareXlaConvParams(OpBuilder &builder, Location loc, ArrayAttr strides,
  ...
  window_strides =
      quant::Create1DConstValue<int32_t>(builder, loc, stride_values);
  lhs_dilation =
      quant::Create1DConstValue<int32_t>(builder, loc, lhs_dilation_values);
  rhs_dilation =
      quant::Create1DConstValue<int32_t>(builder, loc, rhs_dilation_values);

  feature_group_count =
      quant::CreateScalarConstValue<int32_t>(builder, loc, feature_group_cnt);
}
...
Value CreateZeroPointPartialOffset(OpBuilder &builder, Location loc,
  ...
  if (other_tensor_zp == 0) {
    return quant::CreateScalarConstValue<int32_t>(builder, loc, 0);
  }
  ...
  auto reduction_indices_value =
      quant::Create1DConstValue<int64_t>(builder, loc, non_output_indices);
  auto zp =
      quant::CreateScalarConstValue<int32_t>(builder, loc, other_tensor_zp);
  ...
  SmallVector<Value> folded_results = quant::ConstantFoldOpIfPossible(mul_op);
...
Value MergeZeroPointOffset(OpBuilder &builder, Location loc, Value weight,
  ...
  if (!weight_non_output_dynamic_indices.empty()) {
    ...
    auto slice_stride =
        quant::CreateConstValue<int64_t>(builder, loc, {1}, {1});
    for (int64_t weight_idx : weight_non_output_dynamic_indices) {
      auto start =
          quant::CreateConstValue<int64_t>(builder, loc, {1}, {weight_idx});
      auto end =
          quant::CreateConstValue<int64_t>(builder, loc, {1}, {weight_idx + 1});
  ...
  auto zp_offset_value =
      quant::CreateScalarConstValue<int32_t>(builder, loc, zp_constant_offset);
  if (accum_dynamic_dim != nullptr) {
    accum_dynamic_dim =
        builder
            .create<TF::CastOp>(
  ...
  SmallVector<Value> folded_results =
      quant::ConstantFoldOpIfPossible(offset_op);
  return folded_results.front();
}
...
Operation *Create1sMatrix(OpBuilder &builder, Location loc,
  ...
  return builder.create<TF::BroadcastToOp>(
      ...
      quant::CreateConstValue<int32_t>(builder, loc, shape_ones, {1}),
      quant::Create1DConstValue(builder, loc, shape));
}
...
Value CreateXlaConvOp(OpBuilder &builder, Location loc, Value input,
  ...
  if (!quant::GetSplatValue(input_zp, input_zp_value)) {
    emitError(loc,
