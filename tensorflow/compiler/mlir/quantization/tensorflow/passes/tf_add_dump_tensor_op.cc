...
Operation *DuplicateOp(TF::XlaCallModuleOp call_op, PatternRewriter &rewriter,
  ...
  new_call_op->removeAttr(rewriter.getStringAttr(quant::kQuantTraitAttrName));
...
class AddDumpTensorOp : public OpRewritePattern<LiftedOpT> {
 ...
 private:
  ...
  LogicalResult match(LiftedOpT op) const {
    if (!op->hasAttr(quant::kQuantTraitAttrName) || op->getNumResults() != 1) {
      return failure();
    }
    ...
    const FlatSymbolRefAttr f_attr = quant::GetFuncAttr(op);
