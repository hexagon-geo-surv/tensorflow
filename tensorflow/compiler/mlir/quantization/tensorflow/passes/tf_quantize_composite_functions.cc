...
bool IsQuantizedCallforDynamicRange(TF::PartitionedCallOp call_op) {
  ...
  std::unique_ptr<quant::OpQuantSpec> spec = quant::GetTFOpQuantSpec(call_op);
...
LogicalResult TransferTFAttributesToTFUniformAttributes(
  ...
  for (Operation& inner_op : quantized_func.getBody().front().getOperations()) {
    if (auto uniform_op =
        ...
        uniform_op != nullptr) {
      if (failed(quant::FillAttributesForUniformQuantizedConvolutionOp(
      ...
      if (failed(quant::FillAttributesForUniformQuantizedConvolutionOp(
        ...
        return failure();
               ...
               uniform_op != nullptr) {
      if (failed(quant::FillAttributesForUniformQuantizedDotOp(
              rewriter, uniform_op, identifier_to_attr, quantization_method,
              enable_per_channel_quantization)))
        return failure();
    } else if (auto uniform_op =
               ...
               uniform_op != nullptr) {
      if (failed(quant::FillAttributesForUniformQuantizedAddOp(
              rewriter, uniform_op, identifier_to_attr, quantization_method,
              enable_per_channel_quantization)))
        return failure();
    } else if (auto uniform_op =
               ...
               uniform_op != nullptr) {
      if (failed(quant::FillAttributesForUniformQuantizedClipByValueOp(
              rewriter, uniform_op, identifier_to_attr, quantization_method,
              enable_per_channel_quantization)))
        return failure();
    } else if (auto uniform_op =
               ...
               uniform_op != nullptr) {
      if (failed(quant::FillAttributesForUniformRequantizeOp(
              rewriter, uniform_op, identifier_to_attr, quantization_method,
              enable_per_channel_quantization)))
        return failure();
    } else if (auto uniform_op =
               ...
               uniform_op != nullptr) {
      if (failed(quant::FillAttributesForUniformQuantizeOp(
              rewriter, uniform_op, identifier_to_attr, quantization_method,
              enable_per_channel_quantization)))
        return failure();
    }
...
LogicalResult TransferLocation(func::FuncOp float_func,
  ...
  for (Operation& inner_op : float_func.getBody().front().getOperations()) {
    ...
    if (quant::IsOpWithQuantizableTrait(&inner_op)) {
      main_op = &inner_op;
      break;
    }
  }
...
class QuantizeFunctionPattern
 ...
 private:
  ...
  LogicalResult matchAndRewrite(TF::PartitionedCallOp call_op,
    ...
    if (!call_op->removeAttr(quant::kQuantTraitAttrName) || !f_attr) {
      return failure();
    }
...
class QuantizeConstPattern
 ...
 public:
  ...
  explicit QuantizeConstPattern(MLIRContext* context, OpSet target_opset)
      : OpRewritePattern<mlir::quant::ir::QuantizeCastOp>
        (context, /*benefit=*/10),
        target_opset_(target_opset) {}
 ...
 private:
  LogicalResult matchAndRewrite(mlir::quant::ir::QuantizeCastOp q_op,
    ...
    Attribute tensor_proto_attr = quant::Quantize(attr, tensor_qtype);
