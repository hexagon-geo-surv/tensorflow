...
class CalibrationStatisticsSaverOp : public OpKernel {
 public:
  explicit CalibrationStatisticsSaverOp(OpKernelConstruction* context)
      : OpKernel(context) {
  ...
  ~CalibrationStatisticsSaverOp() override {
    ...
    (void)output_file_->Append(statistics_map.SerializeAsString());
    (void)output_file_->Close();
  }
 ...
 private:
  ...
  void AssignIfNotExists(const std::string& id,
                         const CalibrationMethod calibration_method) {
    if (id_to_collector_[id]) return;
