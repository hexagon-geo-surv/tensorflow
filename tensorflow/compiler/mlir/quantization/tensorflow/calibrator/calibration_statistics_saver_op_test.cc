...
TEST_F(CalibrationStatisticsSaverTest, SimpleMinMax) {
  ...
  EXPECT_EQ(statistics_map.statistics().size(), 1);
  EXPECT_TRUE(statistics_map.statistics().contains("1"));

  const CalibrationStatistics& stats = statistics_map.statistics().at("1");
  EXPECT_TRUE(stats.has_min_max_statistics());
...
TEST_F(CalibrationStatisticsSaverTest, SimpleAverageMinMax) {
  ...
  const CalibrationStatistics& stats = statistics_map.statistics().at("1");
  EXPECT_TRUE(stats.has_average_min_max_statistics());
...
TEST_F(CalibrationStatisticsSaverTest, SimpleHistogram) {
  ...
  const CalibrationStatistics& stats = statistics_map.statistics().at("1");
  EXPECT_TRUE(stats.has_histogram_statistics());
...
TEST_F(CalibrationStatisticsSaverTest, MultipleStats) {
  ...
  TF_CHECK_OK(
      ReadBinaryProto(Env::Default(), output_file_path, &statistics_map));
  EXPECT_EQ(statistics_map.statistics().size(), 2);
  EXPECT_TRUE(statistics_map.statistics().contains("1"));
  EXPECT_TRUE(statistics_map.statistics().contains("2"));

  const CalibrationStatistics& stats_1 = statistics_map.statistics().at("1");
  EXPECT_TRUE(stats_1.has_average_min_max_statistics());
  ...
  EXPECT_TRUE(stats_2.has_histogram_statistics());
