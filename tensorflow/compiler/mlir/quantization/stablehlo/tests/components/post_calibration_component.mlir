...
// CHECK-NO-UNPACK: %[[DOT:.+]] = call @quantized_dot_general_fn_1(%[[QUANTIZE_0]], %[[CONST]])
// CHECK-NO-UNPACK: %[[DEQUANTIZE:.+]] = stablehlo.uniform_dequantize %[[DOT]] : (tensor<1x3x!quant.uniform<i8:f32, {{.*}}>>) -> tensor<1x3xf32>
// CHECK-NO-UNPACK: return %[[DEQUANTIZE]] : tensor<1x3xf32>

// CHECK-NO-UNPACK: func.func private @quantized_dot_general_fn_1
// CHECK-NO-UNPACK: %[[DOT_GENERAL_0:.+]] = stablehlo.dot_general
// CHECK-NO-UNPACK-SAME: contracting_dims = [1] x [0] : (tensor<1x1024x!quant.uniform<i8:f32, {{.*}}>>, tensor<1024x3x!quant.uniform<i8<-127:127>:f32, {{.*}}>>) -> tensor<1x3x!quant.uniform<i32:f32, {{.*}}>>
