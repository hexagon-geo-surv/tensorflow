...
#ifndef TENSORFLOW_COMPILER_MLIR_LITE_EXPERIMENTAL_LRT_LITE_RT_COMMON_H_
#define TENSORFLOW_COMPILER_MLIR_LITE_EXPERIMENTAL_LRT_LITE_RT_COMMON_H_
...
typedef enum {
  ...
  // IR related errors.
  kLrtParamIndexOOB = 8,
  kLrtStatusBadTensorType = 9,
  kLrtStatusGraphInvariantError = 10,
} LrtStatusCode;

// Get code from status.
LrtStatusCode CodeFromStatus(LrtStatus status);

// Free any payloads attached to status.
void StatusDestroy(LrtStatus status);

// Create a status with given code.
LrtStatus StatusFromCode(LrtStatusCode code);
...
#endif  // TENSORFLOW_COMPILER_MLIR_LITE_EXPERIMENTAL_LRT_LITE_RT_COMMON_H_
