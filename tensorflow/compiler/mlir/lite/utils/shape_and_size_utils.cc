...
int32_t GetQuantDimensionAfterReshape(
  ...
  while (input_quant_dim > input_dim) {
    if (input_shape[++input_dim] > output_shape[++output_dim]) {
      ...
      // return -1. This is a safety check.
      if (split_output_shape_product != input_shape[input_dim]) return -1;
      ...
      // return -1. This is a safety check.
      if (folded_input_shape_product != output_shape[output_dim]) return -1;
    }
  }
  ...
  if (input_quant_dim < input_dim) return -1;

  // If the input quantization dimension is the same as the input dimension,
  // but the input and output shapes are different, we return -1.
