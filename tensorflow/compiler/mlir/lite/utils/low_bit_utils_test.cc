...
TEST_F(LowBitUtilsTest, Stream4BitValues) {
  ...
  auto apply_chunk_fn = [&](absl::string_view chunk) {
    packed_values.insert(packed_values.end(), chunk.begin(), chunk.end());
...
TEST_F(LowBitUtilsTest, Stream4BitValues8Bit) {
  auto type = mlir::RankedTensorType::get({3}, builder_->getIntegerType(4));
...
TEST_F(LowBitUtilsTest, Stream2BitValues8Bit) {
  auto type = mlir::RankedTensorType::get({7}, builder_->getIntegerType(2));
