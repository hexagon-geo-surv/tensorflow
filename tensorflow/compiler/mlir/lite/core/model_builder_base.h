              ...
              if (vector_size > 0) {
                ...
                flatbuffers::uoffset_t element_offset =
                    flatbuffers::ReadScalar<flatbuffers::uoffset_t>(
                        element_offset_ptr);
                const uint8_t* first_buffer_ptr =
                    element_offset_ptr + element_offset;
                if (first_buffer_ptr >= base &&
                    first_buffer_ptr + sizeof(flatbuffers::soffset_t) <=
                        base + bytes) {
                  flatbuffers::soffset_t b_vtable_offset =
                      flatbuffers::ReadScalar<flatbuffers::soffset_t>(
                          first_buffer_ptr);
                  const uint8_t* b_vtable = first_buffer_ptr - b_vtable_offset;
                  if (b_vtable >= base &&
                      b_vtable + sizeof(flatbuffers::voffset_t) <=
                          base + bytes) {
                    flatbuffers::voffset_t b_vtable_size =
                        flatbuffers::ReadScalar<flatbuffers::voffset_t>(
                            b_vtable);
                    if (b_vtable + b_vtable_size <= base + bytes) {
                      ...
                      if (8 < b_vtable_size) {
                        flatbuffers::voffset_t b_field_offset =
                            flatbuffers::ReadScalar<flatbuffers::voffset_t>(
                                b_vtable + 8);
                        if (b_field_offset != 0 && first_buffer_ptr +
                                                           b_field_offset +
                                                           sizeof(uint64_t) >
                                                       base + bytes) {
