...
Status ParseAndBuildLocation(
  ...
  auto attribute_type_vector = conversion_debug_metadata->attributes_type();
  auto attribute_vector = conversion_debug_metadata->attributes();

  auto& debug_metadata_locations = debug_metadata.debug_metadata_locations;
  debug_metadata_locations.reserve(attribute_type_vector->size());

  // Map index in the attribute_vector to the index in the data structure we are
  // building: DebugMetadata::debug_metadata_locations.
  absl::flat_hash_map<int, int> attribute_location_idx_map;

  for (int i = 0; i < attribute_type_vector->size(); ++i) {
    if (attribute_type_vector->Get(i) == debug_metadata::Attribute_Location) {
      auto location = static_cast<const debug_metadata::Location*>(
          attribute_vector->Get(i));
      if (location->location_type() ==
          debug_metadata::LocationType_FileLineColLoc) {
        // FileLineColLoc.
        auto file_line_col_loc =
            static_cast<const debug_metadata::FileLineColLoc*>(
                location->location());
        debug_metadata_locations.push_back(mlir::FileLineColLoc::get(
            builder.getContext(),
            builder.getStringAttr(file_line_col_loc->filename()->string_view()),
            file_line_col_loc->line(), file_line_col_loc->column()));
      } else if (location->location_type() ==
                 debug_metadata::LocationType_CallSiteLoc) {
        // CallSiteLoc.
        auto callsite_loc = static_cast<const debug_metadata::CallSiteLoc*>(
            location->location());
        if (!attribute_location_idx_map.contains(
                callsite_loc->callee_index()) ||
            !attribute_location_idx_map.contains(
                callsite_loc->caller_index())) {
          return absl::AbortedError("callee or caller index not valid");
        }
        debug_metadata_locations.push_back(mlir::CallSiteLoc::get(
            debug_metadata_locations
                [attribute_location_idx_map[callsite_loc->callee_index()]],
            debug_metadata_locations
                [attribute_location_idx_map[callsite_loc->caller_index()]]));
      } else if (location->location_type() ==
                 debug_metadata::LocationType_NameLoc) {
        // NameLoc.
        auto name_loc =
            static_cast<const debug_metadata::NameLoc*>(location->location());
        if (!attribute_location_idx_map.contains(name_loc->child_index())) {
          return absl::AbortedError("child index not valid");
        }
        debug_metadata_locations.push_back(mlir::NameLoc::get(
            builder.getStringAttr(name_loc->name()->string_view()),
            debug_metadata_locations
                [attribute_location_idx_map[name_loc->child_index()]]));
      } else if (location->location_type() ==
                 debug_metadata::LocationType_UnknownLoc) {
        // UnknownLoc.
        debug_metadata_locations.push_back(
            mlir::UnknownLoc::get(builder.getContext()));
      } else if (location->location_type() ==
                 debug_metadata::LocationType_FusedLoc) {
        // FusedLoc.
        auto fused_loc =
        ...
        for (int fused_loc_idx = 0;
          ...
          if (!attribute_location_idx_map.contains(
            ...
            return absl::AbortedError("location index not valid");
          }
          fused_locations.push_back(
              debug_metadata_locations
                  [attribute_location_idx_map[fused_location_indexes->Get(
                      fused_loc_idx)]]);
        }
        debug_metadata_locations.push_back(mlir::FusedLoc::get(
            fused_locations, mlir::StringAttr::get(builder.getContext(), ""),
            builder.getContext()));
      } else {
        return absl::AbortedError("location type not supported");
      }
  ...
  for (int subgraph_idx = 0; subgraph_idx < subgraphs_debug_metadata->size();
    ...
    for (int operator_idx = 0; operator_idx < operators_debug_metadata->size();
      ...
      // Find the location attribute of the operator. Note that there should be
      // at most one idx pointing to location attribute for each operator.
      ...
      if (!attribute_location_idx_map.contains(location_attribute_idxs[0])) {
        return absl::AbortedError("location attribute index not valid");
      }
      debug_metadata.operator_location_map[subgraph_idx][operator_idx] =
...
// Parse the ConversionDebugMetadata flatbuffer and store debug metadata in
// struct `debug_metadata`.
Status ParseConversionDebugMetadata(
    Builder builder,
    const debug_metadata::ConversionDebugMetadata* conversion_debug_metadata,
    DebugMetadata& debug_metadata) {
  // Parse locations.
  TF_RETURN_IF_ERROR(ParseAndBuildLocation(builder, conversion_debug_metadata,
                                           debug_metadata));
...
Status ParseDebugMetadata(Builder builder, const char* data, size_t size,
  ...
  if (debug_metadata_fb->debug_metadata_type()->size() !=
    ...
    return absl::AbortedError("size not equal");
  }
  ...
  for (int i = 0; i < debug_metadata_fb->debug_metadata_type()->size(); ++i) {
    if (debug_metadata_fb->debug_metadata_type()->Get(i) ==
      ...
      TF_RETURN_IF_ERROR(ParseConversionDebugMetadata(
          builder, conversion_debug_metadata, debug_metadata));
    } else {
      return absl::AbortedError("debug metadata type not supported");
    }
  }
