...
TEST_F(BridgeLoggerFilters, TestEnableOnlyTopLevelPassesFilter) {
  ...
  // By default, enable_only_top_level_passes_ is true.
...
TEST_F(BridgeLoggerFilters, TestEnableOnlyTopLevelPassesEnvVarValues) {
  ...
  // ShouldPrint should return false for nested function since default
  // is enabled.
  EXPECT_FALSE(
  ...
  // ShouldPrint should return true for nested function since filter
  // is disabled.
  EXPECT_TRUE(
      logger_config_zero.ShouldPrint(shape_inference_pass.get(), func_op));

  // Test with environment variable set to "NO".
  setenv("MLIR_BRIDGE_LOG_ENABLE_ONLY_TOP_LEVEL_PASSES", "NO", 1);
  BridgeLoggerConfig logger_config_no;

  // ShouldPrint should return true for nested function since filter
  // is disabled.
  EXPECT_TRUE(
      logger_config_no.ShouldPrint(shape_inference_pass.get(), func_op));
}
...
TEST_F(BridgeLoggerFilters, TestAllFiltersNoMatch) {
  ...
  // match.
  EXPECT_FALSE(logger_config_no_filter.ShouldPrint(shape_inference_pass.get(),
                                                   mlir_module_with_sub.get()));
}
...
TEST_F(BridgeLoggerFilters, TestAllFiltersCombination) {
  ...
  // Test that ShouldPrint returns false if enable_only_top_level_passes_ is
